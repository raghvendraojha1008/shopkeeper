...
interface SubscriptionRecord {
  ...
  graceEndDate: admin.firestore.Timestamp;
  trialUsed:    boolean;
  autoRenew:    boolean;
  paymentId:    string;
  source:       'payment' | 'system' | 'admin';
  introOffer:   boolean;
...
async function fetchPlan(planId: string): Promise<PlanDoc> {
  const snap = await planRef(planId).get();
  if (!snap.exists || !snap.data()?.isActive) {
    throw new HttpsError('not-found', `Plan '${planId}' not found or inactive`);
  }
  return snap.data() as PlanDoc;
}

async function fetchConfig(): Promise<GlobalConfig> {
  const snap = await configRef().get();
  return snap.exists ? { gracePeriodDays: 3, ...(snap.data() as Partial<GlobalConfig>) } : { gracePeriodDays: 3 };
}
...
function buildSubscriptionRecord(opts: {
  ...
  trialUsed?:    boolean;
  existingCreatedAt?: admin.firestore.Timestamp;
}): SubscriptionRecord {
  const now      = Date.now();
  const baseMs   = opts.existingEndMs ? Math.max(now, opts.existingEndMs) : now;
  const endMs    = baseMs + opts.planDoc.durationDays * 86_400_000;
  const graceMs  = endMs  + opts.gracePeriodDays      * 86_400_000;
...
export const confirmManualPayment = onCall(
  { secrets: [] },
  async (request) => {
    if (!request.auth) throw new HttpsError('unauthenticated', 'Login required');
    ...
    if (!planId)         throw new HttpsError('invalid-argument', 'planId required');
    if (!paymentMethod)  throw new HttpsError('invalid-argument', 'paymentMethod required');

    const uid              = request.auth.uid;
    const prefixMap        = { upi: 'upi', bank_transfer: 'bank', cash: 'cash' } as const;
...
export const razorpayWebhook = onRequest(
  { secrets: [RZP_WEBHOOK_SECRET, RZP_KEY_SECRET] },
  async (req, res) => {
