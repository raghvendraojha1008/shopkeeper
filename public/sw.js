...
const CACHE_NAME  = 'shopkeeper-v3';
...
self.addEventListener('fetch', (event) => {
  ...
  // Never cache Vite dev-server internals — they use content-hash URLs that
  // change on every restart/rebuild. Serving stale hashes causes React to load
  // as null (module not found) and crashes the whole app.
  if (
    url.pathname.startsWith('/@vite/') ||
    url.pathname.startsWith('/@fs/') ||
    url.pathname.startsWith('/node_modules/.vite/') ||
    url.searchParams.has('v') && url.pathname.endsWith('.js')
  ) return;
