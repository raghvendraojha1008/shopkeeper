...
const LedgerCard: React.FC<LedgerCardProps> = React.memo(({ l, onEdit, onDelete, onPrint, showDelete = true, searchTerm = '' }) => {
  ...
  const accentColor = isPurchase ? "var(--col-orange)" : "var(--col-blue)";
  const accentBg = isPurchase ? 'rgba(249,115,22,0.1)' : 'var(--col-info-15)';
  const amountColor = isPurchase ? "var(--col-orange-dark)" : "var(--col-blue-700)";
  ...
  return (
    <div className="rounded-[22px] overflow-hidden transition-all active:scale-[0.98] relative"
      style={{
        ...
        border: `1px solid ${isPurchase ? 'rgba(249,115,22,0.25)' : 'var(--col-info-22)'}`,
        boxShadow: `0 4px 20px ${isPurchase ? 'rgba(249,115,22,0.12)' : 'var(--col-info-15)'}, 0 0 0 0 transparent`,
        borderLeft: `3px solid ${accentColor}`,
      }}>
      {/* Sheen */}
      <div className="absolute top-0 left-0 right-0 h-px" style={{background:`linear-gradient(90deg, ${accentColor}40, var(--rgba-white-15), transparent)`}} />
      ...
      <div className="p-4 pl-3.5">
        ...
        <div className="flex justify-between items-start mb-2.5">
          <div className="min-w-0 flex-1 mr-3">
            <div className="flex items-center gap-2 mb-1 flex-wrap">
              <div className="flex items-center gap-1 px-2 py-0.5 rounded-full"
                ...
                <span className="text-app-xs font-black uppercase tracking-wide" style={{color:accentColor}}>
                  {isPurchase ? 'Purchase' : 'Sale'}
                </span>
              </div>
              {displayId && (
                <span className="text-app-xs font-mono px-1.5 py-0.5 rounded-lg"
                  style={{background:'var(--rgba-white-07)', color: 'var(--text-muted)'}}>
                  #{displayId}
                </span>
              )}
              {discount > 0 && (
                <span className="text-app-xs font-black px-1.5 py-0.5 rounded-full flex items-center gap-0.5"
                  style={{background:'var(--col-warning-15)', color:"var(--col-warning)", border:'1px solid var(--col-warning-25)'}}>
                  <BadgePercent size={8} /> Disc
                </span>
              )}
            </div>
            <h3 className="font-black truncate" style={{fontSize:14, letterSpacing:'-0.02em', color: 'var(--text-primary)'}}>
              <Highlighter text={l.party_name} highlight={searchTerm} />
            </h3>
            <div className="flex items-center gap-3 mt-1 text-app-sm" style={{color: 'var(--text-muted)'}}>
          ...
          <div className="flex-shrink-0 text-right">
            ...
            <div className="flex items-center justify-end gap-1 mt-0.5 text-app-xs font-bold"
        ...
        {/* Expanded items */}
        {expanded && l.items && (
          <div className="mb-3 rounded-[14px] overflow-hidden" style={{background:'var(--rgba-white-04)', border:'1px solid var(--glass-border)'}}>
            {l.items.map((item: any, idx: number) => (
              <div key={idx} className="flex justify-between items-center px-3 py-2 text-xs"
                style={{borderBottom: idx < l.items.length-1 ? '1px solid var(--glass-border)' : 'none'}}>
                <span className="font-bold truncate flex-1" style={{color: 'var(--text-primary)'}}>{item.item_name}</span>
                <span className="mx-2 flex-shrink-0" style={{color: 'var(--text-muted)'}}>{item.quantity} {item.unit}</span>
                <span className="font-black tabular-nums flex-shrink-0" style={{color: 'var(--text-primary)'}}>₹{item.rate}</span>
              </div>
            ))}
          </div>
        )}
        ...
        <div className="flex gap-2 pt-2.5" style={{borderTop:'1px solid var(--glass-border)'}}>
          ...
          <button onClick={handleShare}
            className="w-10 h-10 rounded-[14px] flex items-center justify-center active:scale-95 transition-all"
            style={{background:'rgba(37,211,102,0.1)', color:"var(--col-green-light)", border:'1px solid rgba(37,211,102,0.15)'}}>
            <Share2 size={14} />
          </button>
          {showDelete && (
