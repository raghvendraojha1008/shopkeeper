    ...
    <div className="h-full flex flex-col" style={{ background: 'var(--app-bg)' }}>
      ...
      {onBack && (
        <div className="shrink-0 px-4 pt-5 pb-3 flex items-center gap-3 border-b"
          ...
          <p className="font-black text-sm truncate" style={{ color: 'var(--text-primary)' }}>{featureLabel}</p>
        </div>
      )}
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-6 text-center gap-5 pb-16">
        ...
        <div className="relative">
          ...
          <div className="relative w-20 h-20 rounded-[28px] flex items-center justify-center"
            style={{
              background: 'linear-gradient(135deg, var(--col-warning-15), rgba(251,191,36,0.08))',
              border: '1px solid var(--col-warning-35)',
              boxShadow: '0 8px 32px var(--col-warning-15)',
            }}>
            {isExpired && !stillGrace
              ? <Lock size={32} style={{ color: "var(--col-warning)" }} />
              : <Crown size={32} style={{ color: "var(--col-warning)" }} />}
          </div>
        </div>
          ...
          {isExpired && !stillGrace ? (
            <>
              ...
              <p className="text-sm leading-relaxed" style={{ color: 'var(--text-muted)' }}>
                ...
                <span style={{ color: "var(--col-warning)" }}>{featureLabel}</span> and all Pro features.
              </p>
            </>
          ) : (
            <>
        ...
        {benefits.length > 0 && (
          <div className="w-full max-w-sm rounded-[18px] p-4 text-left"
            style={{
              background: 'linear-gradient(135deg, var(--col-warning-07), rgba(251,191,36,0.03))',
              border: '1px solid var(--col-warning-18)',
            }}>
            <p className="text-app-xs font-black uppercase tracking-[0.2em] mb-3"
              style={{ color: 'var(--col-warning-55)' }}>What you unlock</p>
            <div className="space-y-2.5">
              {benefits.map((b, i) => (
                <div key={i} className="flex items-start gap-2.5">
                  <div className="w-4 h-4 rounded-full flex items-center justify-center flex-shrink-0 mt-0.5"
                    style={{ background: 'var(--col-warning-15)' }}>
                    <Crown size={8} style={{ color: "var(--col-warning)" }} />
                  </div>
                  <span className="text-app-md font-bold leading-relaxed"
        ...
        <div className="flex flex-col items-center gap-3 w-full max-w-sm">
          {onGoToSubscription ? (
            <button
              ...
              style={{
                background: 'linear-gradient(135deg, var(--col-warning-22), rgba(251,191,36,0.12))',
                color: "var(--col-warning)",
                border: '1px solid var(--col-warning-40)',
                boxShadow: '0 4px 20px var(--col-warning-12)',
              }}>
              <Crown size={15} />
              Upgrade to Pro
              <ArrowRight size={13} />
