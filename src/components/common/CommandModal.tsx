...
import { 
  ...
  Wallet, Truck, ChevronDown, ChevronUp, Lightbulb, Tag, Wrench
} from 'lucide-react';
...
import { getIDForEntry, seedPartyCounter, generatePartyCode } from '../../utils/idGenerator';
...
const getCollectionMeta = (item: any) => {
  ...
  if (col === 'ledger_entries' && type === 'sell')     return { icon: ShoppingCart,  label: 'Sale',     color: "var(--col-success)", bg: 'var(--col-success-12)' };
  if (col === 'ledger_entries' && type === 'purchase') return { icon: ShoppingBag,   label: 'Purchase', color: "var(--col-amber)", bg: 'var(--col-warning-12)' };
  if (col === 'transactions'   && type === 'received') return { icon: CreditCard,    label: 'Received', color: "var(--col-info)", bg: 'rgba(96,165,250,0.12)' };
  if (col === 'transactions'   && type === 'paid')     return { icon: CreditCard,    label: 'Paid',     color: "var(--col-danger)", bg: 'rgba(248,113,113,0.12)' };
  if (col === 'expenses')   return { icon: Wallet,  label: 'Expense',  color: "var(--col-violet)", bg: 'rgba(167,139,250,0.12)' };
  if (col === 'inventory')  return { icon: Package, label: 'Item',     color: "var(--col-success)", bg: 'var(--col-success-12)' };
  if (col === 'parties')    return { icon: Users,   label: 'Party',    color: "var(--col-indigo)", bg: 'rgba(129,140,248,0.12)' };
  if (col === 'vehicles')     return { icon: Truck,    label: 'Vehicle',  color: "var(--col-warning)", bg: 'rgba(251,191,36,0.12)' };
  if (col === 'misc_charges') return { icon: Tag,      label: 'Charge',   color: "var(--col-orange-400)", bg: 'rgba(251,146,60,0.12)' };
  if (col === 'services')     return { icon: Wrench,   label: 'Service',  color: "var(--col-violet)", bg: 'rgba(167,139,250,0.12)' };
  return { icon: Package, label: 'Entry', color: "var(--col-slate)", bg: 'var(--text-muted)' };
};
...
const CommandModal: React.FC<CommandModalProps> = ({ isOpen, onClose, user, onSuccess, appSettings }) => {
  ...
  const { useParties, useInventory, useServices, invalidateAll } = useData();

  useBackHandler(onClose, isOpen, 10);
  
  const { data: partiesRaw } = useParties(user.uid);

  // Seed party-code counter so AI-created parties receive the correct next ID.
  useEffect(() => {
    const list = (partiesRaw || []) as any[];
    if (list.length > 0) seedPartyCounter(list.map((p: any) => p.party_code));
  }, [partiesRaw]);
  ...
  const buildContext = () => ({
    ...
    itemsRich:    inventory.map(i => ({
      name: i.name, unit: i.unit,
      sale_rate: i.sale_rate, purchase_rate: i.purchase_rate,
      gst_percent: i.gst_percent, price_type: i.price_type,
      hsn_code: i.hsn_code,
    })),
    items:        inventory.map(i => i.name),
    services:     services.map(s => `${s.name}${s.rate_per_unit ? ` ₹${s.rate_per_unit}/${s.unit || 'unit'}` : ''}`),
    vehicles:     (appSettings?.custom_lists?.vehicles || []),
    sites:        (appSettings?.custom_lists?.sites || []),
    ...
    try {
      ...
      for (const cmd of data) {
        ...
        if (collection) {
          ...
          if (collection === 'parties' && !payload.party_code) {
            payload.party_code = generatePartyCode(payload.role === 'supplier' ? 'supplier' : 'customer');
          }
          if (collection === 'ledger_entries' && !payload.invoice_no && !payload.bill_no) {
            if (payload.type === 'sell') payload.invoice_no = getIDForEntry('sell');
            else payload.bill_no = getIDForEntry('purchase');
          }
          if (collection === 'transactions' && !payload.transaction_id) {
            payload.transaction_id = getIDForEntry(payload.type === 'paid' ? 'paid' : 'received');
          }
  ...
  const getModalType = (item: any) => {
    ...
    if (item.collection === 'misc_charges') return 'sales';
    return 'sales';
  };
        ...
        >
          ...
          <div className="flex items-center gap-2 flex-1">
            <div
              ...
              style={{ background: 'var(--col-violet-18)', border: '1px solid var(--col-violet-25)' }}
            >
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
            ...
            <div key={msg.id} className={`flex ${msg.role === 'user' ? 'justify-end' : 'justify-start'}`}>
              {msg.role === 'assistant' && (
                <div
                  ...
                  style={{ background: 'var(--col-violet-18)', border: '1px solid var(--col-violet-25)' }}
                >
                  <Sparkles size={13} style={{ color: "var(--col-violet)" }} />
                </div>
              )}
              ...
              <div className={`max-w-[82%] ${msg.role === 'user' ? '' : 'flex-1'}`}>
                <div
                  ...
                  style={msg.role === 'user'
                    ...
                    : { background: 'var(--rgba-white-05)', border: '1px solid var(--glass-border)' }
                  }
                >
                  {msg.content}
                </div>
                  ...
                  <div className="mt-2 space-y-2">
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center gap-1.5">
                              <span
                                className="text-app-xs font-black uppercase tracking-wide px-1.5 py-0.5 rounded-md"
                                style={{ background: meta.bg, color: meta.color }}
                              >
                                {meta.label}
                              </span>
                            </div>
                            <p className="font-bold text-sm mt-0.5 truncate" style={{ color: 'var(--text-primary)' }}>
                              {label}
                            </p>
                            {item.date && (
                              <p className="text-app-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>{item.date}</p>
                            )}
                          </div>
                          ...
                          <button
                            ...
                            style={{ background: 'var(--col-violet-12)', border: '1px solid var(--col-violet-25)', color: "var(--col-violet)" }}
                          >
                            <Edit2 size={13} />
                          </button>
                        </div>
                    ...
                    <button
                      ...
                      style={{
                        ...
                        boxShadow: '0 4px 16px var(--col-emerald-35)'
                      }}
                    >
          ...
          {aiLoading && (
            <div className="flex justify-start items-center gap-2">
              ...
              >
                <Loader2 size={15} className="animate-spin" style={{ color: "var(--col-violet)" }} />
          ...
          {isFirstMessage && showChips && !aiLoading && (
            <div className="pt-2">
              <div className="flex items-center gap-1.5 mb-2">
                ...
                <span className="text-app-sm font-bold uppercase tracking-wide" style={{ color: 'var(--text-muted)' }}>Try saying</span>
              </div>
              <div className="flex flex-wrap gap-2">
                {QUICK_CHIPS.map((chip, i) => (
                  <button
                    ...
                    className="text-app-md font-semibold px-3 py-1.5 rounded-xl text-left transition-all active:scale-95"
                    style={{
        ...
        >
          ...
          {file && !isRecording && (
            <div className="mb-2 flex items-center gap-2 px-3 py-2 rounded-xl"
              style={{ background: 'var(--col-info-08)', border: '1px solid var(--col-info-25)' }}>
              <ImageIcon size={13} style={{ color: "var(--col-info)" }} />
              <span className="text-xs font-medium flex-1 truncate" style={{ color: "var(--col-info-light)" }}>{file.name}</span>
              <button onClick={() => setFile(null)} style={{ color: "var(--col-danger)" }}><X size={13} /></button>
            </div>
          )}
          ...
          <div className="flex gap-2 items-end">
            ...
            <div
              ...
              style={{ background: 'var(--rgba-white-06)', border: `1px solid ${isRecording ? 'var(--col-danger-35)' :  'var(--rgba-white-12)'}` }}
            >
              ...
              {isRecording && (
                <div className="absolute left-3 top-1/2 -translate-y-1/2 flex items-center gap-1.5 pointer-events-none">
                  ...
                  <span className="text-xs font-semibold tabular-nums" style={{ color: "var(--col-danger-light)" }}>
              ...
              <textarea
                ...
                style={{
                  color: isRecording ? 'transparent' : 'var(--text-primary)',
                  minHeight: '46px',
                  maxHeight: '140px',
                  lineHeight: '24px',
            ...
            {isRecording ? (
              <button
                ...
                style={{
                  background: 'var(--col-danger-18)',
                  border: '1.5px solid var(--col-danger-50)',
              ...
              <button
                ...
                style={{ background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: 'white', boxShadow: '0 3px 12px var(--col-indigo-40)' }}
              >
                <Send size={17} />
              </button>
            ) : micSupported ? (
              <button
                ...
                title="Voice input"
                className="p-2.5 rounded-2xl flex items-center justify-center flex-shrink-0 transition-all active:scale-90"
                style={{ background: 'linear-gradient(135deg,var(--col-accent-75),var(--col-violet-75))', color: 'white', boxShadow: '0 3px 12px var(--col-accent-35)' }}
              >
                <Mic size={17} />
              </button>
            ) : (
          ...
          {/* Hint */}
          <p className="text-center text-app-sm mt-2" style={{ color: 'var(--text-muted)' }}>
            {isRecording
