...
import { getIDForEntry, generatePartyCode, seedPartyCounter } from '../../utils/idGenerator';
...
const CommandModal: React.FC<CommandModalProps> = ({ isOpen, onClose, user, onSuccess, appSettings }) => {
  ...
  // Keep party-code counter seeded so AI-created parties get the correct next P-XXXX
  useEffect(() => {
      ...
      for (const cmd of data) {
        ...
        if (collection) {
          ...
          if (collection === 'parties' && !payload.party_code) {
            payload.party_code = generatePartyCode();
          }
