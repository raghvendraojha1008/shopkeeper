...
const KeyBtn: React.FC<{ label: string; onPress: (n: string) => void; disabled?: boolean }> = ({ label, onPress, disabled }) => (
  <button
    ...
    disabled={disabled}
    className="flex items-center justify-center font-black text-white text-app-d1 rounded-full active:scale-90 transition-all duration-[120ms] disabled:opacity-40"
    style={{
      width: 76, height: 76,
      background: 'var(--rgba-white-08)',
      border: '1px solid var(--glass-border)',
      boxShadow: 'inset 0 1px 0 var(--rgba-white-14), 0 6px 20px var(--rgba-black-35)',
...
const AuroraGlows = () => (
  <>
    <div className="absolute top-[-12%] left-[-8%] w-[75vw] h-[75vw] rounded-full pointer-events-none"
      style={{ background: 'radial-gradient(circle, var(--col-accent-25) 0%, transparent 65%)' }} />
    <div className="absolute bottom-[-8%] right-[-12%] w-[60vw] h-[60vw] rounded-full pointer-events-none"
      style={{ background: 'radial-gradient(circle, var(--col-warning-13) 0%, transparent 65%)' }} />
    <div className="absolute top-[35%] right-[-8%] w-[40vw] h-[40vw] rounded-full pointer-events-none"
      style={{ background: 'radial-gradient(circle, var(--col-violet-12) 0%, transparent 65%)' }} />
  </>
);
...
const LockScreen: React.FC<LockScreenProps> = ({ user, settings, settingsLoaded = false, onPinChanged }) => {
  ...
  if (changePinStep !== 'idle') {
    const cpDotColor = changePinError ? "var(--col-red)" : "var(--col-indigo-500)";
    const cpDotGlow  = changePinError ? 'var(--col-danger-70)' : 'var(--col-accent-70)';
        ...
        <div className="relative flex flex-col items-center w-full max-w-[320px] px-4 my-auto">
          ...
          <div className="mb-5 relative flex items-center justify-center">
            <div className="absolute w-[110px] h-[110px] rounded-full pointer-events-none"
              style={{ background: 'radial-gradient(circle, var(--col-accent-22) 0%, transparent 70%)' }} />
            <div className="relative w-[78px] h-[78px] rounded-[26px] flex items-center justify-center"
              style={{
                background: 'linear-gradient(145deg, #6366f1, #4f46e5)',
                boxShadow: '0 18px 44px rgba(99,102,241,0.38), inset 0 1px 0 var(--rgba-white-20)',
              }}>
              {isEnterStep
          ...
          {/* Title */}
          <h2 className="text-app-d1 font-black text-white mb-1" style={{ letterSpacing: '-0.045em' }}>
            {isEnterStep ? 'Set New PIN' : 'Confirm PIN'}
          </h2>
          <p className="text-app-xl mb-8" style={{ color:  'var(--rgba-white-32)', fontWeight: 500 }}>
          ...
          <div className={`flex gap-5 mb-9 ${changePinShake ? 'animate-[lockshake_0.38s_ease-in-out]' : ''}`}>
            {[0, 1, 2, 3].map(i => (
              <div key={i}
                ...
                style={{
                  width: 14, height: 14,
                  background: i < newPinBuffer.length ? cpDotColor :  'var(--rgba-white-14)',
          ...
          {isEnterStep && newPinBuffer.length >= 2 && (() => {
            ...
            return (
              <p className="text-app-sm font-bold mb-3 -mt-3 text-center" style={{ color: "var(--col-warning)" }}>
          ...
          <div className="w-full flex flex-col gap-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <button
                ...
                style={{ width: 76, height: 76, background: 'var(--rgba-white-04)' }}>
                <Delete size={24} style={{ color:  'var(--rgba-white-45)' }} />
              </button>
            </div>
          </div>
          ...
          <div className="h-7 mt-1 flex items-center justify-center">
            {changePinError && !savingPin && (
              <p className="text-app-md font-black uppercase tracking-[0.18em]" style={{ color: "var(--col-red)" }}>
                {changePinStep === 'confirm' ? "PINs don't match — try again" : 'Failed to save PIN'}
              </p>
            )}
            {savingPin && (
              <p className="text-app-md font-black uppercase tracking-[0.18em]" style={{ color: "var(--col-indigo-500)" }}>
                Saving…
              </p>
            )}
          </div>
          ...
          {!savingPin && (
            <button
              ...
              className="mt-2 text-app-lg font-bold transition-all active:scale-95 py-1"
  ...
  const dotColor = error ? "var(--col-red)" : successAnim ? "var(--col-emerald)" : "var(--col-amber)";
  const dotGlow  = error ? 'var(--col-danger-70)' : successAnim ? 'var(--col-emerald-70)' : 'var(--col-warning-70)';
  ...
  return (
    <div className="lock-screen-root fixed inset-0 z-[9999] flex flex-col items-center justify-center overflow-hidden" style={getScreenStyle(isDarkMode)}>
      ...
      <div className="relative flex flex-col items-center w-full max-w-[320px] px-4 my-auto">
        ...
        <div className="mb-5 relative flex items-center justify-center">
          <div className="absolute w-[110px] h-[110px] rounded-full pointer-events-none"
            style={{ background: 'radial-gradient(circle, var(--col-warning-25) 0%, transparent 70%)' }} />
          <div
            ...
            style={{
              ...
              boxShadow: successAnim
                ? '0 18px 44px var(--col-emerald-45), inset 0 1px 0 var(--rgba-white-20)'
                : '0 18px 44px rgba(245,158,11,0.38), inset 0 1px 0 var(--rgba-white-20)',
        ...
        {/* Title */}
        <h2 className="text-app-d2 font-black text-white mb-1" style={{ letterSpacing: '-0.045em' }}>
        ...
        <div className={`flex gap-5 mb-9 ${shaking ? 'animate-[lockshake_0.38s_ease-in-out]' : ''}`}>
          {[0, 1, 2, 3].map(i => (
            <div key={i}
              ...
              style={{
                width: 14, height: 14,
                background: i < pin.length ? dotColor :  'var(--rgba-white-14)',
        ...
        <div className="w-full relative">
          ...
          {isLockedOut && (
            <div className="absolute inset-0 flex flex-col items-center justify-center rounded-3xl z-20"
              style={{ background: 'var(--modal-bg)', backdropFilter: 'blur(12px)', border: '1px solid var(--col-danger-25)' }}>
              ...
              <p className="text-white font-black text-app-3xl mb-0.5 tracking-tight">Too many attempts</p>
              <p className="text-app-md mb-4" style={{ color: 'var(--text-muted)' }}>
              ...
              <div className="flex items-center gap-2 px-6 py-3 rounded-2xl"
                ...
                <Timer size={18} style={{ color: "var(--col-danger)" }} />
                <span className="text-app-d2 font-black tabular-nums" style={{ color: "var(--col-danger-light)", letterSpacing: '-0.03em' }}>
                  {formatCountdown(timeLeft)}
                </span>
              </div>
              <p className="text-app-sm mt-3 font-semibold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>
        ...
        {!isLockedOut && wrongAttempts >= 2 && user?.email && (
          <button
            ...
            className="mt-1 text-app-lg font-bold transition-all active:scale-95 py-1"
