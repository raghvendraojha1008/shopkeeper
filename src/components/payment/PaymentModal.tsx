...
function CopyBtn({ value }: { value: string }) {
  ...
  return (
    <button
      ...
      style={{ background: 'rgba(99,102,241,0.12)', color: copied ? '#34d399' : '#a78bfa', border: '1px solid rgba(99,102,241,0.2)' }}
    >
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        ...
        <p className="text-[10px] mt-0.5" style={{ color: 'rgba(148,163,184,0.55)' }}>{sub}</p>
      </div>
  ...
  if (step === 'method-select') return (
    <div style={modalStyle} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
      <div style={sheetStyle}>
        ...
        {!hasAnyMethod ? (
          ...
          <div className="flex flex-col gap-3">
            ...
            <div className="flex flex-col gap-1.5">
              <p className="text-center text-[9px] font-black uppercase tracking-[0.15em]"
                style={{ color: 'rgba(148,163,184,0.35)' }}>
                Accepts
              </p>
              <div className="flex flex-wrap gap-2 justify-center">
                {['UPI', 'Card', 'Netbanking', 'EMI', 'Wallet'].map(m => (
                  <span key={m} className="text-[9px] font-black uppercase tracking-[0.12em] px-2.5 py-1 rounded-full"
                    style={{ background: 'rgba(99,102,241,0.08)', color: 'rgba(148,163,184,0.5)', border: '1px solid rgba(99,102,241,0.12)' }}>
                    {m}
                  </span>
                ))}
              </div>
            ...
            <button
              ...
              className="w-full py-4 rounded-[18px] font-black text-[15px] flex items-center justify-center gap-3 active:scale-[0.98] transition-all mt-1"
              style={{ background: 'linear-gradient(135deg, #f59e0b, #fbbf24)', color: '#000', boxShadow: '0 8px 32px rgba(245,158,11,0.35)' }}
            >
              <CreditCard size={20} />
              Pay {price > 0 ? `${symbol}${price.toLocaleString('en-IN')}` : ''} Securely
            </button>

            <p className="text-center text-[10px]" style={{ color: 'rgba(148,163,184,0.4)' }}>
          ...
          <div className="space-y-2.5">
            {activeUpi.length > 0 && (
              <div className="space-y-2">
                {activeUpi.map(entry => (
                  <MethodCard
                    key={entry.id}
                    icon={<Smartphone size={16} style={{ color: '#fbbf24' }} />}
            ...
            {cashEnabled && (
              <MethodCard
                icon={<Banknote size={16} style={{ color: '#fbbf24' }} />}
                label="Cash Payment"
        ...
        <p className="text-center text-[9px] mt-5" style={{ color: 'rgba(148,163,184,0.3)' }}>
      ...
      <div style={modalStyle} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
        <div style={sheetStyle}>
          ...
          <div className="rounded-[18px] p-4 mb-4"
            style={{ background: 'rgba(0,0,0,0.3)', border: '1px solid rgba(251,191,36,0.15)' }}>
            <p className="text-[9px] font-black uppercase tracking-[0.15em] mb-3"
            ...
            <div className="flex items-center gap-2 rounded-[12px] px-3 py-2.5"
              ...
              <p className="text-[13px] font-black text-white flex-1">{selectedUpiEntry.upiId}</p>
              <CopyBtn value={selectedUpiEntry.upiId} />
            </div>
            {selectedUpiEntry.label && (
              <p className="text-[10px] mt-2 ml-1" style={{ color: 'rgba(148,163,184,0.45)' }}>
                {selectedUpiEntry.label}
              </p>
            )}
          </div>
          ...
          <button
            ...
            className="w-full py-3.5 rounded-[16px] font-black text-[13px] flex items-center justify-center gap-2 active:scale-[0.98] transition-all mb-3"
          ...
          <div className="rounded-[14px] px-4 py-3 mb-5"
            ...
            <p className="text-[11px]" style={{ color: 'rgba(226,232,240,0.65)' }}>
          ...
          <button
            ...
            className="w-full py-3.5 rounded-[16px] font-black text-[13px] active:scale-[0.98] transition-all"
          ...
          <div className="rounded-[18px] p-4 mb-4 space-y-2.5"
            ...
            {[
              ...
              <div key={row.label} className="flex items-center gap-2">
                <p className="text-[10px] w-24 shrink-0" style={{ color: 'rgba(148,163,184,0.45)' }}>{row.label}</p>
                <p className="text-[11px] font-bold text-white flex-1 truncate">{row.value}</p>
      ...
      <div style={sheetStyle}>
        <div className="flex flex-col items-center py-10 gap-5">
          <div className="w-14 h-14 rounded-full flex items-center justify-center"
            style={{ background: 'rgba(251,191,36,0.12)' }}>
            <Loader2 size={26} style={{ color: '#fbbf24' }} className="animate-spin" />
          </div>
          <div className="text-center">
            <p className="font-black text-[14px] text-white mb-1">Activating subscription…</p>
    ...
    <div style={modalStyle}>
      <div style={sheetStyle}>
        <div className="flex flex-col items-center py-8 gap-4">
          ...
          <div className="text-center">
            <p className="font-black text-[16px] text-white mb-1">You're on {planName}!</p>
            <p className="text-[11px]" style={{ color: 'rgba(148,163,184,0.5)' }}>
          ...
          <button
            onClick={onClose}
            className="mt-2 w-full py-3.5 rounded-[16px] font-black text-[13px] active:scale-[0.98] transition-all"
      ...
      <div style={modalStyle} onClick={e => { if (e.target === e.currentTarget) onClose(); }}>
        <div style={sheetStyle}>
          <div className="flex flex-col items-center py-8 gap-4">
            ...
            <div className="text-center">
              <p className="font-black text-[14px] text-white mb-1">Activation failed</p>
              <p className="text-[11px] px-4 leading-relaxed" style={{ color: 'rgba(148,163,184,0.55)' }}>
            ...
            <div className="flex flex-col gap-2 w-full mt-1">
              <div className="flex gap-3 w-full">
                <button
                  ...
                  className="flex-1 py-3 rounded-[14px] font-black text-[12px] active:scale-[0.98] transition-all"
                  style={{ background: 'rgba(255,255,255,0.06)', border: '1px solid rgba(255,255,255,0.1)', color: 'rgba(226,232,240,0.7)' }}
                >
                  Try Again
                </button>
                <button
              ...
              {mailtoHref && (
                <a
                  href={mailtoHref}
                  className="flex items-center justify-center gap-2 w-full py-3 rounded-[14px] font-black text-[12px] active:scale-[0.98] transition-all"
                  style={{ background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.25)', color: '#a78bfa', textDecoration: 'none' }}
                >
                  <Mail size={13} />
                  Contact Support
                </a>
