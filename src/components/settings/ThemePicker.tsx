...
export const ThemePicker: React.FC<Props> = ({ value, onChange, custom, onChangeCustom }) => {
  ...
  return (
    <div className="space-y-3">
      ...
      {current === 'custom' && (
        <div className="rounded-xl border border-white/10 bg-[var(--rgba-white-05)] p-3 space-y-3">
          <div className="flex items-center justify-between">
            <div>
              <div className="text-xs font-extrabold text-foreground">Custom color</div>
              <div className="text-app-sm text-muted-foreground">Adjust primary accent (HSL)</div>
            </div>
            <div
...
const SliderRow = ({
  ...
  return (
    <div className="grid grid-cols-[48px_1fr_42px] items-center gap-3">
      <div className="text-app-md font-extrabold text-[var(--text-muted)]">{label}</div>
      <input
        type="range"
        min={min}
        max={max}
        ...
        className="w-full"
      />
      <div className="text-app-md font-black text-[var(--text-primary)] text-right tabular-nums">{value}</div>
    </div>
  );
};
