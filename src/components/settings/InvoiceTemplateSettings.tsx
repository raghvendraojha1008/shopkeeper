...
const InvoiceTemplateSettings: React.FC<InvoiceTemplateSettingsProps> = ({ settings, onUpdateSettings }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      {/* Template picker */}
      <Section id="template" title="Template Style" icon={<FileText size={16} style={{ color: '#a78bfa' }} />} open={section === 'template'} onToggle={() => toggleSection('template')}>
      ...
      {/* Colors & Header */}
      <Section id="colors" title="Colors & Header" icon={<Palette size={16} style={{ color: '#60a5fa' }} />} open={section === 'colors'} onToggle={() => toggleSection('colors')}>
      ...
      {/* Logo & Branding */}
      <Section id="branding" title="Logo & Branding" icon={<Building2 size={16} style={{ color: '#34d399' }} />} open={section === 'branding'} onToggle={() => toggleSection('branding')}>
        <div className="pt-2 space-y-3">
          {/* Logo upload */}
          <div>
          ...
          <div className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-2">
                <p className="text-[10px] font-black uppercase tracking-wide" style={{ color: 'rgba(148,163,184,0.4)' }}>Base Font Size</p>
                <span className="text-xs font-black" style={{ color: '#a78bfa' }}>{tpl.base_font_size ?? 12} pt</span>
              </div>
              ...
              <div className="flex justify-between text-[9px] mt-1" style={{ color: 'rgba(148,163,184,0.35)' }}>
                <span>8 pt (compact)</span><span>12 pt (default)</span><span>16 pt (large)</span>
              </div>
              <p className="text-[9px] mt-1" style={{ color: 'rgba(148,163,184,0.4)' }}>All text in the PDF scales proportionally from this base size.</p>
            </div>
            {/* Font family */}
            <div>
              <p className="text-[10px] font-black uppercase tracking-wide mb-2" style={{ color: 'rgba(148,163,184,0.4)' }}>Font Style</p>
              <div className="flex gap-2">
                {[{ id: 'helvetica', label: 'Sans-Serif', sample: 'Aa' }, { id: 'times', label: 'Serif', sample: 'Aa' }, { id: 'courier', label: 'Monospace', sample: 'Aa' }].map(f => (
                  <button key={f.id} onClick={() => update('font_family', f.id)}
                    ...
                    <div className="text-[9px] font-black mt-0.5">{f.label}</div>
                  </button>
                ))}
              </div>
            </div>
      ...
      {/* Field toggles */}
      <Section id="fields" title="Fields & Sections" icon={<Sliders size={16} style={{ color: '#fbbf24' }} />} open={section === 'fields'} onToggle={() => toggleSection('fields')}>
      ...
      {/* Terms & bank details */}
      <Section id="content" title="Terms & Bank Details" icon={<CreditCard size={16} style={{ color: '#f97316' }} />} open={section === 'content'} onToggle={() => toggleSection('content')}>
        <div className="pt-2 space-y-3">
          {tpl.show_terms && (
            <div>
