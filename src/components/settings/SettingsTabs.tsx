...
import { 
    ...
    Zap, Home, Calendar, LayoutList, Package
} from 'lucide-react';
    ...
    return (
        <div className="space-y-4 animate-in slide-in-from-right duration-300">
            <SettingsSection title="App Preferences" icon={Store}>
                ...
                {/* ── Smart Item Sync ────────────────────────────────────── */}
                <div className="border-b border-white/07 pb-4 pt-4">
                    <div className="flex items-center justify-between p-2">
                        <div className="flex items-center gap-3">
                            <div className="p-2 rounded-lg bg-[rgba(16,185,129,0.15)] text-[#34d399]"><Package size={20}/></div>
                            <div>
                                <div className="font-bold text-sm text-[rgba(226,232,240,0.88)]">Smart Item Sync</div>
                                <div className="text-xs text-[rgba(148,163,184,0.45)]">Update item rates, GST & unit in stock after each order</div>
                            </div>
                        </div>
                        <label className="relative inline-flex items-center cursor-pointer">
                            <input
                                type="checkbox"
                                className="sr-only peer"
                                checked={formData.automation?.auto_sync_item_info !== false}
                                onChange={e => updateAutomation({ auto_sync_item_info: e.target.checked })}
                            />
                            <div className="w-11 h-6 bg-[rgba(255,255,255,0.1)] border border-white/10 peer-focus:outline-none rounded-full peer peer-checked:after:translate-x-full after:content-[''] after:absolute after:top-[2px] after:left-[2px] after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-emerald-500"></div>
                        </label>
                    </div>
                    {formData.automation?.auto_sync_item_info !== false && (
                        <div className="mt-2 mx-2 p-3 rounded-xl" style={{ background: 'rgba(16,185,129,0.06)', border: '1px solid rgba(16,185,129,0.15)' }}>
                            <p className="text-[10px] font-bold text-[rgba(148,163,184,0.5)] uppercase mb-2">Fields to sync</p>
                            <div className="grid grid-cols-2 gap-y-2 gap-x-3">
                                {([
                                    { key: 'sale_rate',     label: 'Sale Rate',      hint: 'from Sales orders'     },
                                    { key: 'purchase_rate', label: 'Purchase Rate',   hint: 'from Purchase orders'  },
                                    { key: 'gst_percent',   label: 'GST %',           hint: 'incl. rate type'       },
                                    { key: 'unit',          label: 'Unit',            hint: 'Pcs / Kg / Ltr …'      },
                                ] as const).map(({ key, label, hint }) => {
                                    const checked = (formData.automation?.sync_item_fields?.[key]) !== false;
                                    return (
                                        <label key={key} className="flex items-start gap-2 cursor-pointer">
                                            <div
                                                className="mt-0.5 flex items-center justify-center rounded flex-shrink-0 transition-colors"
                                                style={{
                                                    width: 14, height: 14,
                                                    background: checked ? 'rgba(16,185,129,0.8)' : 'rgba(255,255,255,0.06)',
                                                    border: `1px solid ${checked ? 'rgba(16,185,129,0.9)' : 'rgba(255,255,255,0.2)'}`,
                                                }}>
                                                <input
                                                    type="checkbox"
                                                    className="hidden"
                                                    ...
                                                    onChange={e => updateAutomation({
                                                        sync_item_fields: {
                                                            ...(formData.automation?.sync_item_fields || {}),
                                            ...
                                            <div>
                                                <div className="text-xs font-bold text-[rgba(226,232,240,0.8)]">{label}</div>
                                                <div className="text-[10px] text-[rgba(148,163,184,0.4)]">{hint}</div>
                                            </div>
                                        </label>
                                    );
                                })}
