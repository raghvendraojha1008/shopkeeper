...
export const SettingsSection = ({ title, icon: Icon, children, className = '' }: any) => (
  <div className={`rounded-2xl p-5 border border-white/10 ${className}`}>
    <div className="flex items-center gap-2 mb-4 font-bold text-xs uppercase tracking-wider text-[var(--text-muted)]">
...
export const SettingInput = ({ label, value, onChange, placeholder, type = "text", disabled = false, icon: Icon }: any) => (
    <div className="relative group">
        <label className="text-app-sm font-bold text-slate-400 uppercase mb-1.5 block ml-1">{label}</label>
        <div className="relative">
            {Icon && <div className="absolute left-3 top-3 text-[var(--text-muted)]"><Icon size={16}/></div>}
            <input 
                type={type}
                value={value}
                ...
                disabled={disabled}
                className={`w-full  border border-white/12 rounded-xl py-3 ${Icon ? 'pl-10' : 'pl-4'} pr-4 text-sm font-bold text-[var(--text-secondary)] outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent transition-all disabled:opacity-50`}
            />
        </div>
    </div>
);
