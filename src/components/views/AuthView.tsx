...
const AuthView = () => {
  ...
  return (
    <div className="flex flex-col items-center justify-center p-6 relative safe-area-bottom" style={{background: 'var(--app-bg)', minHeight: '100dvh'}}>
      <div className="bg-[var(--rgba-white-06)] w-full max-w-md rounded-3xl shadow-xl p-8 z-10 border border-[var(--rgba-white-10)]">
        ...
        <h1 className="text-3xl font-black text-center text-[var(--text-primary)] mb-2 tracking-tight">{isLogin ? 'Welcome Back' : 'Join Now'}</h1>
        ...
        <div className="relative my-8"><div className="absolute inset-0 flex items-center"><div className="w-full border-t border-slate-200"></div></div><div className="relative flex justify-center text-app-sm uppercase tracking-widest"><span className="bg-[var(--rgba-white-06)] px-2 text-slate-400 font-bold">Or continue with</span></div></div>
        <button onClick={handleGoogle} disabled={loading} className="w-full bg-[var(--rgba-white-06)] border border-[var(--rgba-white-10)] text-[var(--text-primary)] py-3.5 rounded-xl font-bold text-sm active:scale-95 transition-all flex items-center justify-center gap-3">{loading ? <Loader2 className="animate-spin text-slate-400"/> : <img src="https://www.google.com/favicon.ico" className="w-5 h-5" alt="G"/>} Continue with Google</button>
      ...
      <div className="mt-8 text-slate-400 text-app-sm font-black uppercase tracking-widest opacity-50">Secured by Shop Ledger</div>
    </div>
  );
};
