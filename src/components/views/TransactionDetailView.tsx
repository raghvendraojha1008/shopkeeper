...
import {
  ...
  CreditCard, CheckCircle2, Copy, Link2, Package, IndianRupee,
} from 'lucide-react';
...
const TransactionDetailView: React.FC<TransactionDetailViewProps> = ({
  transaction: txn, settings, ledgerEntries = [], onBack, onEdit,
}) => {
  ...
  const accentColor  = isReceived ? "var(--col-success)" : "var(--col-danger)";
  const accentBg     = isReceived ? 'var(--col-emerald-15)' : 'var(--col-danger-08)';
  const accentBorder = isReceived ? 'var(--col-emerald-25)' : 'var(--col-danger-22)';
  const typeLabel    = isReceived ? 'Money Receipt' : 'Payment Voucher';

  // Linked order — look up by bill_no across ledger entries
  const linkedOrder = txn.bill_no
    ? ledgerEntries.find((e: any) =>
        String(e.invoice_no || e.bill_no || '').trim() === String(txn.bill_no).trim()
      ) || null
    : null;
  ...
  const generatePDF = async () => {
    ...
    doc.text(`${fmtINR(amount, 'Rs.')}`, PW/2, 89, {align:'center'});
      ...
      <div className="px-4 pt-3 pb-32 space-y-3">
        ...
        <div className="rounded-[24px] overflow-hidden relative"
          ...
          <div className="p-5 space-y-4">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-app-sm font-black uppercase tracking-wider" style={{ color: accentColor }}>
                {typeLabel}
              </span>
              <span className="ml-auto text-app-xs font-bold px-2.5 py-1 rounded-xl"
            ...
            <div className="text-center py-3">
              <p className="text-app-d7 font-black tabular-nums leading-none" style={{ color: accentColor, letterSpacing: '-0.03em' }}>
                <span style={{ fontSize: '50%', opacity: 0.5 }}>₹</span>
                {Math.round(amount).toLocaleString('en-IN')}
              </p>
              <p className="text-app-sm mt-1.5 italic" style={{ color: 'var(--text-muted)' }}>
            ...
            <div className="rounded-[16px] p-3.5"
              ...
              <div className="flex items-center gap-2.5">
                <div className="p-2 rounded-[12px]" style={{ background: 'var(--rgba-white-07)' }}>
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="text-sm font-black" style={{ color: "var(--col-warning)" }}>
                {txn.payment_mode || 'Cash'}
              </span>
              {txn.payment_purpose && (
                <span className="text-app-sm px-2.5 py-1 rounded-xl ml-auto"
                  style={{ background: 'var(--rgba-white-06)', color: 'var(--text-muted)' }}>
                  {txn.payment_purpose}
                </span>
              )}
        ...
        {linkedOrder && (
          <div className="rounded-[20px] overflow-hidden"
            style={{ background: 'var(--col-accent-08)', border: '1px solid var(--col-accent-22)' }}>
            <div className="px-4 pt-3.5 pb-1 flex items-center gap-2">
              <Link2 size={11} style={{ color: "var(--col-indigo)" }} />
              <p className="text-app-xs font-black uppercase tracking-wider" style={{ color: "var(--col-indigo)" }}>
                Connected {linkedOrder.type === 'sell' ? 'Sale' : 'Purchase'} Order
              </p>
            </div>
            <div className="px-4 pb-4 space-y-2 mt-1">
              <div className="flex items-center justify-between">
                <span className="text-sm font-black text-white">
                  {linkedOrder.party_name || '—'}
                </span>
                <span className="text-app-sm font-bold px-2 py-0.5 rounded-lg"
                  style={{ background: 'var(--col-accent-15)', color: "var(--col-indigo-light)" }}>
              ...
              <div className="grid grid-cols-3 gap-2 mt-1">
                <div className="rounded-[10px] p-2 text-center"
                  ...
                  <p className="text-app-2xs font-bold uppercase mb-0.5" style={{ color: 'var(--text-muted)' }}>Total</p>
                  <p className="text-app-md font-black tabular-nums" style={{ color: "var(--col-slate-200)" }}>
                    {fmtINR(Number(linkedOrder.total_amount) || 0)}
                  </p>
                </div>
                <div className="rounded-[10px] p-2 text-center"
                  style={{ background: 'var(--rgba-white-05)' }}>
                  <p className="text-app-2xs font-bold uppercase mb-0.5" style={{ color: 'var(--text-muted)' }}>Date</p>
                  <p className="text-app-sm font-bold" style={{ color: "var(--col-slate)" }}>
                    {String(linkedOrder.date || '—').substring(0, 10)}
                  </p>
                </div>
                <div className="rounded-[10px] p-2 text-center"
                  style={{ background: 'var(--rgba-white-05)' }}>
                  <p className="text-app-2xs font-bold uppercase mb-0.5" style={{ color: 'var(--text-muted)' }}>Items</p>
                  <p className="text-app-md font-black" style={{ color: "var(--col-slate-200)" }}>
              ...
              {(linkedOrder.items || []).length > 0 && (
                <div className="space-y-1 mt-0.5">
                  {(linkedOrder.items || []).slice(0, 3).map((item: any, idx: number) => (
                    <div key={idx} className="flex items-center justify-between text-app-sm">
                      <span style={{ color: 'var(--text-muted)' }}>
                        {item.item_name} <span style={{ color: 'var(--text-muted)' }}>×{item.quantity} {item.unit || ''}</span>
                      </span>
                      <span className="font-bold tabular-nums" style={{ color: 'var(--text-secondary)' }}>
                        {fmtINR(Number(item.quantity) * Number(item.rate))}
                      </span>
                    </div>
                  ))}
          ...
          {[
            ...
            <div key={i} className="flex items-center gap-3">
              ...
              <div className="flex items-center gap-1.5 flex-1 justify-end">
                <p className="text-app-md font-bold" style={{ color: 'var(--text-secondary)' }}>{value}</p>
                {copyable && (
                  <button onClick={copyTxnId} className="active:scale-90 transition-all">
                    {copied ? <CheckCircle2 size={12} style={{ color: "var(--col-success)" }} />
