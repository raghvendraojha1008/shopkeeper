...
const ItemDetailView: React.FC<ItemDetailViewProps> = ({ user, item, onBack }) => {
  ...
  // DataContext cached collections — zero extra Firestore reads on repeated item opens.
  const { useLedger, useParties } = useData();
  const { data: allLedger = [], isLoading: loading } = useLedger(user.uid);
  const { data: allParties = [] } = useParties(user.uid);
  ...
  // Ledger entries for this item — filtered from DataContext cache.
  const ledgerEntries = useMemo(() => {
    const itemName = item.name?.toLowerCase();
    return allLedger
      .filter((entry: any) =>
        entry.items?.some((i: any) => i.item_name?.toLowerCase() === itemName)
      )
      .slice()
      .sort((a: any, b: any) => {
  ...
  // Party suggestions based on tab — from DataContext cache
  const searchSuggestions = useMemo(() => {
    return allParties.filter((p: any) => {
    ...
    <div className="h-full overflow-y-auto" style={{background: 'var(--app-bg)'}}>
      ...
      <div className="px-3 md:px-6">
        ...
        <div className="flex justify-between items-start">
          <div className="flex-1">
            <div className="flex items-center gap-2 mb-2">
              ...
              <span className="text-xs font-bold text-[var(--text-muted)]">Current Stock</span>
            </div>
            <div className="text-2xl font-black ">
              {item.current_stock} <span className="text-sm text-[var(--text-muted)]">{item.unit}</span>
            </div>
          </div>
          <div className="flex gap-4 text-right">
            <div>
              <div className="flex items-center gap-1 text-app-sm text-slate-400 font-bold justify-end">
                <TrendingDown size={10} className="text-green-500" /> Buy
              </div>
              <div className="font-bold text-[var(--text-secondary)]">₹{item.purchase_rate || 0}</div>
            </div>
            <div>
              <div className="flex items-center gap-1 text-app-sm text-slate-400 font-bold justify-end">
        ...
        {item.hsn_code && (
          <div className="mt-2 pt-2 border-t border-dashed border-white/08 flex gap-2">
            <span className="text-app-sm text-slate-400 text-[var(--text-secondary)] px-2 py-0.5 rounded font-bold">
              HSN: {item.hsn_code}
            </span>
            <span className="text-app-sm text-slate-400 text-[var(--text-secondary)] px-2 py-0.5 rounded font-bold">
      ...
      <div className={`p-4 rounded-2xl shadow-lg mb-3 flex justify-between items-center relative overflow-hidden ${
        ...
        <div className="relative z-10">
          <div className="text-app-sm font-bold opacity-70 uppercase mb-0.5">
          ...
          <div className="flex gap-3">
            <div className="flex items-center gap-1.5 bg-[var(--rgba-white-06)]/10 px-2 py-1 rounded-lg">
              <Package size={12} />
              <span className="text-app-sm font-bold uppercase">
              ...
              <div className="pl-2">
                ...
                <div className="flex items-center justify-between mb-1">
                  <div className="flex items-center gap-2">
                    <span className="text-app-sm font-bold text-[var(--text-muted)]">{entry.date}</span>
                    {entry.invoice_no && (
                      <span className="text-app-xs font-bold text-slate-400 px-1.5 py-0.5 rounded flex items-center gap-1">
                        <FileText size={8}/> #{entry.invoice_no}
                      </span>
                    )}
                  </div>
                  <span className={`text-app-xs font-black uppercase ${
                ...
                <div className="flex justify-between items-center mb-2">
                  ...
                  <div className="text-right">
                    <div className="font-black text-base text-[var(--text-secondary)] text-[var(--text-primary)]">
                ...
                <div className="overflow-x-auto pt-2 border-t border-dashed border-white/08 -mx-1 px-1">
                  <div className="flex items-center gap-3 text-app-md text-slate-500 whitespace-nowrap min-w-0">
                    <div className="flex items-center gap-1 flex-shrink-0">
                      <Package size={12} className="" style={{color: "var(--text-muted)"}}/>
