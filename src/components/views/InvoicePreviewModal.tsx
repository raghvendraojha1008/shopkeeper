...
const InvoicePreviewModal: React.FC<InvoicePreviewModalProps> = ({ invoice, settings, parties = [], onClose }) => {
  ...
  // Android back closes the modal (always mounted = always active)
  ...
  // PDF currency formatter — uses Rs. prefix since jsPDF built-in fonts lack the ₹ glyph
  ...
  const generateStandardPDF = (): jsPDF => {
    ...
    const primaryColor = tpl.theme_color || "var(--col-blue-900)";
    const rgb = hexToRgb(primaryColor) || [30, 58, 138];
    const fontFamily: string = tpl.font_family || 'helvetica';
    const invoiceTitle: string = tpl.invoice_title || 'INVOICE';

    // Font sizes scaled from base_font_size setting
    const base: number = tpl.base_font_size ?? 12;
    const FS = {
      h1: Math.round(base * 1.33),   // ~16
      h2: Math.round(base * 1.83),   // ~22
      body: Math.round(base * 0.75), // ~9
      small: Math.round(base * 0.67),// ~8
      tiny: Math.round(base * 0.58), // ~7
      grand: Math.round(base * 1.0), // ~12
    };
    ...
    doc.setFont(fontFamily, 'bold');
    doc.text(invoiceTitle, pageWidth - margin, 35, { align: 'right' });
    ...
    const body = calculatedData.items.map((item: any) => {
      const row: any[] = [
        item.item_name,
        `${item.qty} ${item.unit || ''}`,
        pdfCurrency(Number(item.rate)),
        isGstEnabled ? `${item.gstPercent}%` : null,
        pdfCurrency(Number(item.total)),
      ];
    ...
    autoTable(doc, {
      ...
      body,
      theme: 'grid',
      headStyles: { fillColor: rgb, textColor: 255, fontStyle: 'bold', fontSize: FS.body, font: fontFamily, halign: 'left' },
      bodyStyles: { fontSize: FS.small, cellPadding: 3, font: fontFamily, halign: 'left' },
      columnStyles: {
    ...
    if (isGstEnabled) {
      ...
      doc.text(pdfCurrency(calculatedData.totalTaxable), pageWidth - margin, finalY, { align: 'right' });
      finalY += 5;
      doc.text('Total Tax:', totalsX, finalY);
      doc.text(pdfCurrency(calculatedData.totalTax), pageWidth - margin, finalY, { align: 'right' });
      finalY += 5;
    }
    if (includeRent && calculatedData.vehicleRent > 0) {
      doc.text('Transport:', totalsX, finalY);
      doc.text(pdfCurrency(calculatedData.vehicleRent), pageWidth - margin, finalY, { align: 'right' });
      finalY += 5;
    }
    ...
    doc.setFontSize(FS.grand);
    doc.setFont(fontFamily, 'bold');
    doc.setTextColor(rgb[0], rgb[1], rgb[2]);
    doc.text('GRAND TOTAL:', totalsX, finalY);
    doc.text(pdfCurrency(calculatedData.grandTotal), pageWidth - margin, finalY, { align: 'right' });
    ...
    // Terms & Conditions block (between amount-in-words and the footer)
    const termsRaw: string = tpl.terms_text || '';
    if (termsRaw) {
      const termsStartY = finalY + 14;
      doc.setFontSize(FS.tiny);
      doc.setFont(fontFamily, 'bold');
      doc.setTextColor(100);
      ...
      // Strip leading "Terms & Conditions:" prefix to avoid printing it twice
      const termsBody = termsRaw.replace(/^Terms\s*&\s*Conditions\s*:?\s*\n?/i, '').trim();
      const wrapped = doc.splitTextToSize(termsBody || termsRaw, pageWidth - margin * 2 - 45);
      doc.text(wrapped, margin, termsStartY + 4);
    }

    // Footer: authorized signatory + signature line
    const pageHeight = doc.internal.pageSize.height;
    const signatoryLabel: string = tpl.authorized_signatory || 'Authorized Signatory';
    doc.setFontSize(FS.tiny);
    doc.setFont(fontFamily, 'normal');
    doc.setTextColor(120);
    doc.setDrawColor(180);
    doc.line(pageWidth - margin - 48, pageHeight - 14, pageWidth - margin, pageHeight - 14);
    doc.setFont(fontFamily, 'bold');
    doc.text(signatoryLabel, pageWidth - margin, pageHeight - 9, { align: 'right' });
  ...
  return (
    <div className="fixed inset-0 bg-black/80 z-[80] flex justify-center items-end md:items-center animate-in fade-in duration-200">
      <div className="bg-[var(--app-bg)] w-full md:max-w-2xl rounded-t-2xl md:rounded-2xl shadow-2xl flex flex-col max-h-[95vh] md:h-[85vh]">
        {/* Header (unchanged) */}
        <div className="p-4 border-b border-white/08 rounded-t-2xl flex justify-between items-center shrink-0">
          <h2 className="font-bold text-lg text-[var(--text-primary)]">Preview</h2>
            ...
            <div className="flex p-0.5 rounded-lg">
              {(['standard', 'thermal58', 'thermal80'] as PrintMode[]).map(mode => (
                <button
                  ...
                  className={`px-2 py-1 text-app-xs font-bold rounded transition-all ${
            ...
            {calculatedData.vehicleRent > 0 && (
              <button
                ...
                className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${
                  includeRent ? 'bg-[var(--col-info-25)] text-blue-400' : 'bg-[var(--rgba-white-07)] text-[var(--text-muted)]'
                }`}
              >
        ...
        <div className="flex-1 overflow-auto bg-col-app-bg-deep p-4">
          <div className={`shadow-lg mx-auto rounded-lg p-6 text-xs md:text-sm bg-[var(--modal-bg)] text-[var(--text-primary)] ${
            ...
            {printMode !== 'standard' ? (
              <div className="text-center font-mono">
                ...
                {settings.profile?.contact && <p className="text-app-sm">Ph: {settings.profile.contact}</p>}
                <div className="border-t border-dashed border-slate-300 my-2" />
                <div className="flex justify-between text-app-sm">
                  <span>Bill #: {invoice.invoice_no || '-'}</span>
                  <span>{invoice.date}</span>
                </div>
                <div className="text-left text-app-sm mt-1">To: {invoice.party_name}</div>
                <div className="border-t border-slate-300 my-2" />
                <div className="grid grid-cols-4 text-app-xs font-bold border-b border-slate-200 pb-1">
                ...
                {calculatedData.items.map((it: any, i: number) => (
                  <div key={i} className="grid grid-cols-4 text-app-xs py-0.5">
                ...
                <div className="border-t border-dashed border-slate-300 my-2" />
                <p className="text-app-xs">Thank You!</p>
              </div>
            ) : (
              <>
                {/* Standard invoice preview content unchanged */}
                <div className="border-b-2 border-[var(--rgba-white-08)] pb-4 mb-4">
                  ...
                  {isGstEnabled && settings.profile?.gstin && (
                    <span className="inline-block mt-1.5 text-app-sm font-mono font-bold bg-[var(--col-info-12)] text-col-info-light px-2 py-1 rounded">
                      GSTIN: {settings.profile.gstin}
                    </span>
                  )}
                </div>
                <div className="grid grid-cols-2 gap-4 mb-6">
                  <div className="p-3 rounded-lg border border-[var(--rgba-white-08)] bg-[var(--rgba-white-04)]">
                    <div className="text-app-sm text-slate-400 font-bold uppercase mb-1">Bill To</div>
                  ...
                  <div className="text-right">
                    <div className="text-2xl font-black text-[var(--text-muted)]">INVOICE</div>
                    <div className="font-bold">#{invoice.invoice_no || '-'}</div>
                    <div className="text-slate-500">{invoice.date}</div>
                  </div>
                </div>
                <table className="w-full text-left mb-6">
                  <thead className="text-app-sm uppercase bg-[var(--col-violet-25)]">
                    <tr>
                  ...
                  <tbody className="divide-y divide-slate-100 text-app-md">
                    {calculatedData.items.map((it: any, i: number) => (
                      <tr key={i}>
                ...
                <div className="flex justify-end">
                  <div className="text-right w-1/2">
                    {isGstEnabled && (
                      <>
                        <div className="flex justify-between text-app-md text-slate-500 mb-1">
                    ...
                    {calculatedData.vehicleRent > 0 && (
                      <div className={`flex justify-between text-app-md mb-2 ${includeRent ? 'font-bold' : 'line-through'}`}>
