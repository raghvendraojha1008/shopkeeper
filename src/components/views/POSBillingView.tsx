...
import { getIDForEntry, peekNextID, confirmID } from '../../utils/idGenerator';
import {
  buildInvoiceData,
...
const POSBillingView: React.FC<POSBillingViewProps> = ({ user, appSettings, onBack }) => {
  ...
  const [rawInvoiceNo] = useState<string>(() => peekNextID('sales'));
  ...
  const persist = async (): Promise<any | null> => {
    ...
    try {
      ...
      // Pre-generate the ledger entry document ID client-side BEFORE any write.
      // This makes the write idempotent: if the Firestore SDK's in-memory pending
      // write (queued when navigator.onLine lies on Android WebView) and a
      // SyncQueueService replay both fire when connectivity returns, they both
      // call setDoc to the SAME document — identical overwrite, no duplicate.
      const ledgerDocId = generateDocId();
      ...
      //
      // Both paths use ledgerDocId (pre-generated) so the write is idempotent:
      // if the Firestore SDK's in-memory write and a SyncQueueService replay
      // race, they target the same document — no duplicate.
      if (isOffline) {
        SyncQueueService.addToQueue(user.uid, 'create', 'ledger_entries', payload, ledgerDocId);
        payload.id = ledgerDocId;
        showToast('Saved offline — will sync', 'info');
      } else {
        try {
          await ApiService.setDocById(user.uid, 'ledger_entries', ledgerDocId, payload);
          payload.id = ledgerDocId;
          ...
          if (writeErr?.code === 'WRITE_TIMEOUT') {
            // navigator.onLine reported true but network is actually down
            // (common on Android WebView).  The Firestore SDK already has
            // this write in its in-memory queue and will complete it when
            // connectivity returns.  Queue to SyncQueueService with the SAME
            // pre-generated ledgerDocId so if both paths fire on reconnect
            // they converge on the same document — idempotent, no duplicate.
            SyncQueueService.addToQueue(user.uid, 'create', 'ledger_entries', payload, ledgerDocId);
            payload.id = ledgerDocId;
            setLedgerCache(old => old.map(e => e.id === tempId ? { ...payload } : e));
        ...
        await Promise.all(items.map(async (li: any) => {
          ...
          if (isOffline) {
            ...
            try {
              ...
              if (writeErr?.code === 'WRITE_TIMEOUT') {
                // Same idempotency logic — update uses docId so replaying is safe.
                SyncQueueService.addToQueue(user.uid, 'update', 'inventory', { current_stock: newStock }, match.id);
              } else {
                throw writeErr;
              }
    ...
    <div className="h-full overflow-hidden flex flex-col" style={{ background: 'var(--app-bg)' }}>
      ...
      <div className="sticky top-0 z-30 px-4 pb-3 flex-shrink-0"
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1 min-w-0">
            <p className="text-app-xs font-black uppercase tracking-[0.2em]"
          ...
          <div className="text-right flex-shrink-0">
            <p className="text-app-2xs font-black uppercase tracking-wider"
      ...
      <div className="flex-1 overflow-y-auto px-4 pt-3 pb-3 space-y-3">
        ...
        <div ref={partyBoxRef} className="relative rounded-[16px] p-3 space-y-2"
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-app-xs font-black uppercase tracking-[0.15em]"
              style={{ color: 'var(--text-muted)' }}>Customer</p>
            {partyName && (
              <button
                onClick={() => {
                ...
                className="ml-auto text-app-xs font-bold flex items-center gap-1 px-2 py-0.5 rounded-full active:scale-95"
          ...
          <input
            ...
            className="w-full px-3 py-2 rounded-xl text-app-xl font-bold text-white outline-none"
          ...
          {partyName && !showCustomerExtras && !partyGstin && !partyPhone && (
            <button
              ...
              className="text-app-sm font-bold tracking-wide active:scale-95"
          ...
          {partyName && (showCustomerExtras || partyGstin || partyPhone) && (
            <div className="grid grid-cols-2 gap-2">
              <input
                ...
                className="px-3 py-2 rounded-xl text-app-lg font-bold text-white outline-none tabular-nums tracking-wide"
                style={{ background: 'var(--rgba-white-05)', border: '1px solid var(--glass-border)' }}
              />
              <input
                value={partyPhone}
                ...
                className="px-3 py-2 rounded-xl text-app-lg font-bold text-white outline-none"
                style={{ background: 'var(--rgba-white-05)', border: '1px solid var(--glass-border)' }}
              />
              ...
              {partyGstin.length === 15 && interstate && (
                <p className="col-span-2 text-app-sm font-bold flex items-center gap-1"
          ...
          {showPartyList && partySuggestions.length > 0 && (
            <div className="pos-suggestion-list absolute left-3 right-3 top-full mt-1 z-20 rounded-xl overflow-hidden max-h-60 overflow-y-auto"
              style={{ background: "var(--col-navy)", border: '1px solid var(--glass-border)', boxShadow: '0 8px 24px var(--rgba-black-40)' }}>
              {partySuggestions.map((p: any) => (
                <button key={p.id || p.name}
                  ...
                  <p className="text-app-lg font-bold text-white truncate">{p.name}</p>
                  {(p.gstin || p.contact || p.phone) && (
                    <p className="text-app-sm tabular-nums" style={{ color: 'var(--text-muted)' }}>
        ...
        <div className="relative rounded-[16px] p-3 space-y-2"
          ...
          <div className="flex items-center gap-2">
            ...
            <p className="text-app-xs font-black uppercase tracking-[0.15em]"
              style={{ color: 'var(--text-muted)' }}>Add Item</p>
          </div>
          <div className="flex gap-2">
            <input
              ...
              className="flex-1 px-3 py-2.5 rounded-xl text-app-xl font-bold text-white outline-none"
              style={{ background: 'var(--rgba-white-05)', border: '1px solid var(--glass-border)' }}
            />
            <button onClick={() => setShowScanner(true)}
              className="px-3 rounded-xl active:scale-95 transition-all flex items-center gap-1.5"
              style={{
                background: 'var(--col-accent-18)',
                border: '1px solid var(--col-accent-35)',
                color: "var(--col-indigo-light)",
              }}>
              <ScanLine size={14} />
              <span className="text-app-sm font-black uppercase tracking-wider">Scan</span>
            </button>
          </div>
          {itemSuggestions.length > 0 && (
            <div className="pos-suggestion-list rounded-xl max-h-[220px] overflow-y-auto"
              onMouseDown={e => e.preventDefault()}
              style={{ background: "var(--col-navy)", border: '1px solid var(--glass-border)' }}>
              {itemSuggestions.map((it: any) => (
                <button key={it.id || it.name}
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="text-app-lg font-bold text-white truncate">{it.name}</p>
                    <div className="flex gap-2 text-app-xs font-bold mt-0.5">
                      <span style={{ color: 'var(--text-muted)' }}>{it.unit || 'Pcs'}</span>
                      <span style={{ color: Number(it.current_stock) <= Number(it.min_stock || 0) ? "var(--col-warning)" : 'var(--text-muted)' }}>
                        Stock: {Number(it.current_stock) || 0}
                      </span>
                    </div>
                  </div>
                  <div className="text-right flex-shrink-0">
                    <p className="text-app-lg font-black tabular-nums" style={{ color: "var(--col-success)" }}>
          ...
          {search && itemSuggestions.length === 0 && (
            <p className="text-app-md font-bold text-center py-2"
        ...
        <div className="rounded-[16px] overflow-hidden"
          ...
          <div className="px-3 py-2.5 flex items-center justify-between"
            ...
            <div className="flex items-center gap-2">
              ...
              <p className="text-app-xs font-black uppercase tracking-[0.15em]"
                style={{ color: 'var(--text-muted)' }}>Cart</p>
            </div>
            <p className="text-app-sm font-bold" style={{ color: 'var(--text-muted)' }}>
              {totals.qtyCount} {totals.qtyCount === 1 ? 'unit' : 'units'}
              {cart.length > 0 && (
                <span className="ml-2 hidden sm:inline" style={{ color: 'var(--text-muted)' }}>
          ...
          {cart.length === 0 ? (
            <div className="px-4 py-8 text-center">
              <Package size={28} className="mx-auto mb-2" style={{ color: 'var(--text-muted)' }} />
              <p className="text-app-md font-bold" style={{ color: 'var(--text-muted)' }}>
            ...
            cart.map((l, idx) => {
              ...
              return (
                <div key={l.key} className="px-3 py-3 space-y-2"
                  style={{ borderBottom: idx < cart.length - 1 ? '1px solid var(--glass-border)' : 'none' }}>
                  ...
                  <div className="flex items-start justify-between gap-2">
                    ...
                    >
                      <p className="text-app-lg font-bold text-white truncate">{l.item_name}</p>
                      <div className="flex items-center gap-2 mt-0.5 text-app-xs font-bold"
                        style={{ color: 'var(--text-muted)' }}>
                        <span>{l.unit}</span>
                        ...
                        {typeof l.stockOnHand === 'number' && l.quantity > l.stockOnHand && (
                          <span style={{ color: "var(--col-warning)" }}>⚠ over stock</span>
                        )}
                      </div>
                    </div>
                    <button onClick={() => setEditKey(l.key)}
                      ...
                      style={{ background: 'var(--col-accent-14)', color: "var(--col-indigo-light)" }}>
                      <Pencil size={12} />
                    </button>
                    <button onClick={() => removeLine(l.key)}
                      className="p-1.5 rounded-lg active:scale-95"
                      style={{ background: 'var(--col-danger-12)', color: "var(--col-danger)" }}>
                      <Trash2 size={12} />
                    </button>
                  </div>
                  {/* line 2: qty stepper row */}
                  <div className="flex items-center gap-2">
                    <span className="text-app-xs font-bold w-8 flex-shrink-0"
                      style={{ color: 'var(--text-muted)' }}>Qty</span>
                    <div className="flex items-center rounded-xl overflow-hidden flex-1"
                      ...
                      <input
                        ...
                        className="flex-1 min-w-0 text-center text-app-xl font-black text-white bg-transparent outline-none tabular-nums"
                      />
                      <button onClick={() => incQty(l.key)}
                        className="px-2.5 py-1.5 active:scale-90"
                        style={{ color: 'var(--col-success-95)' }}>
                        <Plus size={12} />
                      </button>
                    </div>
                  </div>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-1 px-2 py-1.5 rounded-xl"
                      ...
                      <span className="text-app-sm font-bold flex-shrink-0" style={{ color: 'var(--text-muted)' }}>₹</span>
                      <input
                        type="number"
                        value={l.rate}
                        onChange={e => updateLine(l.key, { rate: Math.max(0, Number(e.target.value) || 0) })}
                        className="flex-1 min-w-0 text-app-lg font-bold text-white bg-transparent outline-none tabular-nums"
                      />
                    </div>
                    <p className="flex-1 text-right text-app-xl font-black tabular-nums"
      ...
      <div className="pt-2"
        ...
        <button
          ...
          style={{
            background: 'linear-gradient(135deg, var(--col-emerald-15), var(--col-emerald-05))',
            border: '1px solid var(--col-emerald-35)',
          }}>
          <div className="flex items-center gap-2">
            <span className="text-app-sm font-black uppercase tracking-[0.12em]"
              style={{ color: 'rgba(110,231,183,0.8)' }}>Grand Total</span>
            {(totals.totalCgst > 0 || totals.totalIgst > 0) && (
              <span className="text-app-xs font-bold px-1.5 py-0.5 rounded-full"
                style={{ background: 'var(--col-emerald-18)', color: 'rgba(110,231,183,0.7)' }}>
                +GST
              </span>
            )}
          </div>
          <div className="flex items-center gap-2">
            <span className="text-xl font-black tabular-nums" style={{ color: "var(--col-success)" }}>
        ...
        {totalsExpanded && (
          <div className="rounded-[14px] p-3 mb-2"
            style={{
              background: 'var(--col-emerald-06)',
              border: '1px solid var(--col-emerald-18)',
            }}>
            <div className="flex justify-between text-app-md font-bold mb-1"
              style={{ color: 'var(--text-muted)' }}>
              <span>Subtotal</span>
            ...
            <button
              ...
              className="flex items-center gap-1.5 text-app-xs font-bold uppercase tracking-wider mt-1 active:scale-95"
              style={{ color: 'var(--text-muted)' }}>
              <span
                className="inline-block w-7 h-3.5 rounded-full transition-colors"
                style={{
                  background: roundToRupee ? 'var(--col-emerald-50)' :   'var(--rgba-white-10)',
                  position: 'relative',
                }}>
                <span
        ...
        <div className="grid grid-cols-2 gap-2 mb-2">
          <button
            ...
            className="flex items-center justify-center gap-2 py-3 rounded-[16px] text-app-lg font-black active:scale-95 transition-all"
            style={{
          ...
          <button
            ...
            disabled={saving || cart.length === 0}
            className="flex items-center justify-center gap-2 py-3 rounded-[16px] text-app-lg font-black text-white active:scale-95 transition-all"
            style={{
              background: 'linear-gradient(135deg, #10b981, #059669)',
              boxShadow: '0 6px 20px var(--col-emerald-35)',
              opacity: saving || cart.length === 0 ? 0.45 : 1,
            }}>
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            disabled={saving || cart.length === 0}
            className="flex items-center justify-center gap-1.5 py-2 rounded-[14px] text-app-md font-black active:scale-95 transition-all"
            style={{
      ...
      {editingLine && (
        <div
          ...
          style={{ background: 'var(--rgba-black-55)', backdropFilter: 'blur(6px)' }}
          ...
          >
            ...
            <div className="grid grid-cols-2 gap-2">
              <div className="rounded-2xl p-3 space-y-2"
                style={{ background: 'var(--rgba-white-04)', border: '1px solid var(--glass-border)' }}>
                <p className="text-app-xs font-black uppercase tracking-wider"
                  style={{ color: 'var(--text-muted)' }}>Quantity</p>
                <div className="flex items-center gap-2">
                  <button onClick={() => updateLine(editingLine.key, { quantity: Math.max(0, editingLine.quantity - 1) })}
                    ...
                    style={{ background: 'rgba(248,113,113,0.15)', color: "var(--col-danger)" }}>
                    <Minus size={16} />
                  </button>
                  <input
                    type="number"
                  ...
                  <button onClick={() => updateLine(editingLine.key, { quantity: editingLine.quantity + 1 })}
                    className="w-9 h-9 rounded-xl active:scale-90 flex items-center justify-center"
                    style={{ background: 'var(--col-success-15)', color: "var(--col-success)" }}>
                    <Plus size={16} />
                  </button>
                </div>
              </div>
              <div className="rounded-2xl p-3 space-y-2"
                style={{ background: 'var(--rgba-white-04)', border: '1px solid var(--glass-border)' }}>
                <p className="text-app-xs font-black uppercase tracking-wider"
                  style={{ color: 'var(--text-muted)' }}>Rate (₹)</p>
                <input
                  type="number"
                  inputMode="decimal"
            ...
            <div className="grid grid-cols-3 gap-2">
              <div className="rounded-2xl p-2.5 space-y-1.5"
                style={{ background: 'var(--rgba-white-04)', border: '1px solid var(--glass-border)' }}>
                <p className="text-app-xs font-black uppercase tracking-wider"
                  style={{ color: 'var(--text-muted)' }}>GST %</p>
                <input
                  type="number"
                  inputMode="decimal"
              ...
              <div className="rounded-2xl p-2.5 space-y-1.5"
                style={{ background: 'var(--rgba-white-04)', border: '1px solid var(--glass-border)' }}>
                <p className="text-app-xs font-black uppercase tracking-wider"
                  style={{ color: 'var(--text-muted)' }}>Unit</p>
                <input
              ...
              <div className="rounded-2xl p-2.5 space-y-1.5"
                style={{ background: 'var(--rgba-white-04)', border: '1px solid var(--glass-border)' }}>
                <p className="text-app-xs font-black uppercase tracking-wider"
                  style={{ color: 'var(--text-muted)' }}>HSN</p>
                <input
            ...
            <div className="flex items-center gap-2 rounded-2xl p-1"
              ...
              {(['exclusive', 'inclusive'] as const).map(pt => (
                <button key={pt}
                  ...
                  className="flex-1 py-2 rounded-xl text-app-md font-black uppercase tracking-wider transition-all"
                  style={{
                    background: editingLine.price_type === pt
                      ? 'linear-gradient(135deg, var(--col-accent-35), var(--col-violet-25))'
                      : 'transparent',
                    color: editingLine.price_type === pt ? 'white' : 'var(--text-muted)',
                    border: editingLine.price_type === pt
                      ? '1px solid var(--col-violet-40)' : '1px solid transparent',
                  }}>
            ...
            <div className="flex items-center justify-between px-2 py-2 rounded-xl"
              style={{ background: 'var(--col-emerald-08)', border: '1px solid var(--col-emerald-25)' }}>
              <span className="text-app-sm font-black uppercase tracking-wider"
                style={{ color: 'rgba(110,231,183,0.8)' }}>Line total</span>
              <span className="text-base font-black tabular-nums" style={{ color: "var(--col-success)" }}>
            ...
            <div className="grid grid-cols-2 gap-2 pt-1">
              <button
                ...
                className="flex items-center justify-center gap-2 py-3 rounded-2xl text-app-lg font-black active:scale-95"
                style={{ background: 'var(--col-danger-12)', border: '1px solid var(--col-danger-25)', color: "var(--col-danger)" }}>
                <Trash2 size={14} /> Remove
              </button>
              <button
                onClick={() => { setEditKey(null); refocusSearch(); }}
                className="flex items-center justify-center gap-2 py-3 rounded-2xl text-app-lg font-black text-white active:scale-95"
                style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', boxShadow: '0 6px 20px var(--col-accent-35)' }}>
                <Check size={14} /> Done
              </button>
            </div>
          </div>
