...
const SalesDashboard: React.FC<SalesDashboardProps> = ({ user, onBack }) => {
  const scrollRef = useScrollMemory('sales-dashboard');
  ...
  return (
    <div className="flex flex-col h-full px-3 pt-3 md:px-6" style={{background: 'var(--app-bg)'}}>
      ...
      {/* TOTAL CARD */}
      <div className="p-5 rounded-2xl shadow-lg mb-4 flex justify-between items-center relative overflow-hidden shrink-0" style={{background:"linear-gradient(135deg,var(--col-emerald-85),rgba(5,150,105,0.9))"}}>
           <div className="relative z-10">
               <div className="text-app-sm font-bold opacity-80 uppercase mb-1">Total Sales Volume (Excl. Rent)</div>
      ...
      <div className="p-2.5 rounded-xl mb-4 space-y-2 bg-[var(--rgba-white-04)] border border-white/08">
           <div className="flex gap-2">
               <div className="flex-1 relative">
                   ...
                   <input 
                       className="w-full pl-8 p-2 border border-white/12 bg-[var(--rgba-white-05)] rounded-lg text-xs font-bold outline-none text-[var(--text-secondary)]" 
      ...
      <div ref={scrollRef} className="flex-1 overflow-y-auto pb-20 space-y-2">
          {loading ? <div className="text-center py-10 text-[var(--text-muted)]">Loading...</div> : stats.finalItems.map((item: any, i: number) => (
              <div key={i} className="p-4 rounded-xl flex justify-between items-center bg-[var(--rgba-white-05)] border border-white/08">
                  ...
                  <div className="text-right">
                      <div className="text-base font-black text-col-success">
          ...
          {stats.finalItems.length === 0 && (
              <div className="text-center py-10 text-[var(--text-muted)] text-xs">No items sold in this period.</div>
          )}
      </div>
    </div>
  );
