...
import { seedPartyCounter, generatePartyCode } from '../../utils/idGenerator';
...
const GCard: React.FC<{ children: React.ReactNode }> = ({ children }) => (
  <div className="rounded-2xl p-4" style={{ background: 'var(--rgba-white-04)', border: '1px solid var(--glass-border)' }}>{children}</div>
);
...
const BulkImportView: React.FC<Props> = ({ user, settings, onBack }) => {
  ...
  const TYPE_INFO: Record<ImportType, { Icon: any; label: string; desc: string; color: string; bg: string; border: string }> = {
    inventory:    { Icon: Package,      label: 'Inventory Items',              desc: 'Name, rate, stock, HSN, GST type',        color: "var(--col-success)", bg: 'var(--col-emerald-12)',  border: 'var(--col-emerald-25)'  },
    parties:      { Icon: Users,        label: 'Parties (Customers/Suppliers)', desc: 'Customers & suppliers with GSTIN, state', color: "var(--col-info)", bg: 'var(--col-info-12)',  border: 'var(--col-info-25)'  },
    sale:         { Icon: TrendingUp,   label: 'Sale Invoices',                desc: 'Date, party, items, amount, vehicle',     color: "var(--col-violet)", bg: 'var(--col-violet-12)', border: 'var(--col-violet-25)' },
    purchase:     { Icon: TrendingDown, label: 'Purchase Bills',               desc: 'Date, supplier, items, amount, vehicle',  color: "var(--col-amber)", bg: 'var(--col-warning-12)', border: 'var(--col-warning-25)' },
    transactions: { Icon: Wallet,       label: 'Payments (Transactions)',      desc: 'Payments received & paid with UTR',       color: "var(--col-warning)", bg: 'var(--col-warning-12)', border: 'var(--col-warning-25)' },
    expenses:     { Icon: Receipt,      label: 'Expenses',                     desc: 'Date, category, amount',                  color: "var(--col-danger)", bg: 'var(--col-danger-12)',  border: 'var(--col-danger-25)'  },
    vehicles:     { Icon: Truck,        label: 'Vehicles',                     desc: 'Vehicle number, driver details',           color: "var(--col-sky)", bg: 'rgba(56,189,248,0.12)', border: 'rgba(56,189,248,0.25)' },
  };
  ...
  const runImport = async () => {
    ...
    try {
      // Seed counter before assigning IDs to bulk-imported parties
      if (type === 'parties') {
        try {
          const existingParties = await ApiService.getAll(user.uid, 'parties');
          seedPartyCounter(existingParties.map((p: any) => p.party_code));
        } catch (_) {}
      }
      for (let i = 0; i < validRows.length; i++) {
        try {
          const row = transformRow(validRows[i].mapped, type);
          if (type === 'parties' && !row.party_code) row.party_code = generatePartyCode(row.role === 'supplier' ? 'supplier' : 'customer');
          await ApiService.add(user.uid, COLLECTION[type], row);
          success++;
        }
  ...
  // Per-step key: each wizard step gets its own scroll memory so navigating
  // back to a previous step restores where the user was on that step.
  const scrollRef = useScrollMemory(`bulk-import-${step}`);

  return (
    <div ref={scrollRef} className="h-full overflow-y-auto pb-28" style={{ background: 'var(--app-bg)' }}>
      {/* Header */}
      <div className="sticky top-0 z-30 px-4 pb-3" style={{paddingTop: '16px',  background: 'rgba(var(--app-bg-rgb),0.96)', backdropFilter: 'blur(20px)', borderBottom: '1px solid var(--glass-border)' }}>
        <div className="flex items-center gap-3 mb-3">
          ...
          <div>
            ...
            <p className="text-app-xs font-bold uppercase tracking-widest" style={{ color: 'var(--text-muted)' }}>CSV · Excel · Google Sheets</p>
          </div>
        </div>
        {/* Step bar */}
        <div className="flex items-center gap-1">
          {STEPS.map((s, i) => (
            <React.Fragment key={s}>
              <div className="flex items-center gap-1 px-2 py-1 rounded-lg text-app-xs font-black transition-all"
                style={i === si ? { background: ti.bg, color: ti.color, border: `1px solid ${ti.border}` }
                     : i < si  ? { background: 'var(--col-success-15)', color: "var(--col-success)" }
                                : { color: 'var(--text-muted)' }}>
                {i < si && '✓ '}{s}
              </div>
              {i < 4 && <div className="flex-1 h-px" style={{ background: i < si ? "var(--col-success)" : 'var(--rgba-white-07)' }} />}
            </React.Fragment>
          ))}
        </div>
      </div>
      ...
      <div className="px-4 pt-4 space-y-4">
        ...
        {step === 'type' && (
          <>
            ...
            <div className="flex items-center gap-3 p-4 rounded-2xl" style={{ background: 'linear-gradient(135deg, var(--col-indigo-15), var(--col-violet-15))', border: '1px solid var(--col-violet-35)' }}>
              <div className="p-2.5 rounded-xl flex-shrink-0" style={{ background: 'var(--col-violet-25)', border: '1px solid var(--col-violet-35)' }}>
                <BookOpen size={20} style={{ color: "var(--col-violet)" }} />
              </div>
              <div className="flex-1">
                <p className="text-sm font-black text-white">Import Guide & Templates</p>
                <p className="text-app-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>Download the complete guide with all columns explained + sample data for every import type</p>
              </div>
              <button onClick={downloadImportGuide} className="flex items-center gap-1.5 px-3 py-2.5 rounded-xl text-xs font-black flex-shrink-0 active:scale-95"
                style={{ background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: 'white', boxShadow: '0 4px 12px var(--col-indigo-40)' }}>
                <FileDown size={14} /> Download
              </button>
            </div>

            {/* AI mapping info banner */}
            <div className="flex items-center gap-3 p-3 rounded-2xl" style={{ background: isGeminiReady ? 'var(--col-violet-08)' : 'var(--rgba-white-04)', border: `1px solid ${isGeminiReady ? 'var(--col-violet-25)' : 'var(--rgba-white-08)'}` }}>
              <Sparkles size={16} style={{ color: isGeminiReady ? "var(--col-violet)" : 'var(--text-muted)', flexShrink: 0 }} />
              <p className="text-app-sm font-semibold" style={{ color: isGeminiReady ? 'rgba(167,139,250,0.8)' : 'var(--text-muted)' }}>
                {isGeminiReady
            ...
            <p className="text-sm font-bold" style={{ color: 'var(--text-muted)' }}>What would you like to import?</p>
            {(Object.entries(TYPE_INFO) as [ImportType, typeof ti][]).map(([t, m]) => {
              ...
              return (
                <button key={t} onClick={() => setType(t)} className="w-full flex items-center gap-4 p-4 rounded-2xl active:scale-[0.98] transition-all"
                  style={sel ? { background: m.bg, border: `1.5px solid ${m.border}` } : { background: 'var(--rgba-white-04)', border: '1px solid var(--glass-border)' }}>
                  <div className="w-12 h-12 rounded-2xl flex items-center justify-center flex-shrink-0" style={{ background: sel ? `${m.color}22` : 'var(--rgba-white-06)', border: `1px solid ${sel ? m.border : 'var(--rgba-white-08)'}` }}>
                    <m.Icon size={22} style={{ color: sel ? m.color : 'var(--text-muted)' }} />
                  </div>
                  <div className="text-left flex-1">
                    <p className="font-black text-sm" style={{ color: sel ? m.color : 'var(--text-secondary)' }}>{m.label}</p>
                    <p className="text-app-sm font-semibold mt-0.5" style={{ color: 'var(--text-muted)' }}>{m.desc}</p>
                  </div>
            ...
            <button onClick={() => setStep('upload')} className="w-full py-4 rounded-2xl font-black text-sm flex items-center justify-center gap-2 active:scale-[0.97]"
              style={{ background: 'linear-gradient(135deg,#4f46e5,#7c3aed)', color: 'white', boxShadow: '0 8px 24px var(--col-indigo-40)' }}>
          ...
          <>
            <GCard>
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <p className="text-app-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>Fill this CSV and upload below</p>
                </div>
              ...
              <div className="mt-3 rounded-xl overflow-hidden" style={{ border: '1px solid var(--glass-border)' }}>
                <div className="px-3 py-1 text-app-2xs font-bold uppercase tracking-widest" style={{ background: 'var(--rgba-white-04)', borderBottom: '1px solid var(--glass-border)', color: 'var(--text-muted)' }}>
                  Sample format
                </div>
                <pre className="px-3 py-2 text-app-xs font-mono overflow-x-auto" style={{ color: 'var(--text-muted)' }}>
            ...
            <div onDrop={onDrop} onDragOver={e => e.preventDefault()} onClick={() => fileRef.current?.click()}
              ...
              <div className="w-16 h-16 rounded-2xl flex items-center justify-center" style={{ background: `${ti.color}18`, border: `1px solid ${ti.color}33` }}>
                {aiMapping
                  ? <Loader2 size={28} className="animate-spin" style={{ color: "var(--col-violet)" }} />
              ...
              <div className="text-center">
                ...
                <p className="text-app-md mt-1" style={{ color: 'var(--text-muted)' }}>
            ...
            {isGeminiReady && (
              <div className="flex items-center gap-2 px-4 py-2.5 rounded-xl" style={{ background: 'var(--col-violet-08)', border: '1px solid var(--col-violet-25)' }}>
                <Sparkles size={13} style={{ color: "var(--col-violet)", flexShrink: 0 }} />
                <p className="text-app-sm font-semibold" style={{ color: 'rgba(167,139,250,0.75)' }}>
            ...
            <GCard>
              <p className="text-app-xs font-black uppercase tracking-widest mb-2.5" style={{ color: 'var(--text-muted)' }}>How to export from your app</p>
              {[['Google Sheets','File → Download → CSV'],['Excel','File → Save As → CSV (Comma Delimited)'],['Numbers','File → Export To → CSV'],['OpenOffice','File → Save As → Text CSV']].map(([app, s]) => (
                <div key={app} className="flex gap-2 py-1">
                  ...
                  <p className="text-app-sm font-semibold" style={{ color: 'var(--text-muted)' }}><span className="font-black" style={{ color: 'var(--text-primary)' }}>{app}: </span>{s}</p>
                </div>
              ))}
            </GCard>

            <button onClick={() => setStep('type')} className="w-full py-3 rounded-2xl font-black text-sm active:scale-95"
              style={{ background: 'var(--rgba-white-07)', color: 'var(--text-muted)' }}>← Back</button>
          </>
        )}
            ...
            <GCard>
              <div className="flex items-center justify-between mb-3">
                <div>
                  ...
                  <p className="text-app-sm mt-0.5" style={{ color: 'var(--text-muted)' }}>{rawRows.length} rows in your file</p>
                </div>
                <span className="text-app-sm font-black px-2.5 py-1.5 rounded-xl" style={{ background: ti.bg, color: ti.color, border: `1px solid ${ti.border}` }}>
                ...
                <div className="mb-3 flex items-center gap-2 px-3 py-2.5 rounded-xl"
                  style={{
                    background: aiMapped
                      ...
                      : aiError
                        ? 'var(--col-danger-08)'
                        : 'var(--rgba-white-05)',
                    border: `1px solid ${aiMapped ? 'var(--col-violet-35)' : aiError ? 'var(--col-danger-25)' :   'var(--rgba-white-10)'}`,
                  }}>
                  {aiMapping
                    ? <Loader2 size={13} className="animate-spin flex-shrink-0" style={{ color: "var(--col-violet)" }} />
                    : aiMapped
                      ? <Sparkles size={13} style={{ color: "var(--col-violet)", flexShrink: 0 }} />
                      : <AlertTriangle size={13} style={{ color: "var(--col-danger)", flexShrink: 0 }} />
                  }
                  <p className="text-app-sm font-semibold flex-1"
                    style={{ color: aiMapped ? 'rgba(167,139,250,0.85)' : aiError ? 'rgba(248,113,113,0.8)' : 'var(--text-muted)' }}>
                    {aiMapping
                  ...
                  {!aiMapping && !aiMapped && isGeminiReady && (
                    <button onClick={retryAiMapping}
                      className="flex items-center gap-1 px-2 py-1 rounded-lg text-app-xs font-black flex-shrink-0 active:scale-95"
                ...
                return (
                  <div key={col.key} className="flex items-center gap-2 mb-2.5">
                    <div className="w-36 flex-shrink-0">
                      <p className="text-app-sm font-black" style={{ color: hasMissing ? "var(--col-danger)" : 'var(--text-secondary)' }}>
                        {col.label}{col.required ? ' *' : ''}
                      </p>
                      {col.hint && <p className="text-app-xs" style={{ color: 'var(--text-muted)' }}>{col.hint}</p>}
                    </div>
                    <div className="flex-1 relative">
                      <select value={mapping[col.key] || ''} onChange={e => setMapping(m => ({ ...m, [col.key]: e.target.value }))}
                        className="w-full px-2 py-1.5 rounded-lg text-app-sm font-bold outline-none"
                        style={{
                          background: isAiMappedCol ? 'var(--col-violet-15)' : 'var(--rgba-white-07)',
                          border: `1px solid ${hasMissing ? 'var(--col-danger-40)' : isAiMappedCol ? 'var(--col-violet-35)' :  'var(--rgba-white-12)'}`,
                      ...
                      {isAiMappedCol && (
                        <Sparkles size={10} style={{ color: "var(--col-violet)", position: 'absolute', right: 6, top: '50%', transform: 'translateY(-50%)', pointerEvents: 'none' }} />
                      )}
                    </div>
                  </div>
                );
          ...
          <>
            ...
            <GCard>
              <div className="flex gap-3">
                <div className="flex-1 text-center p-2 rounded-xl" style={{ background: 'var(--col-success-15)', border: '1px solid var(--col-success-25)' }}>
                  <p className="text-lg font-black" style={{ color: "var(--col-success)" }}>{validRows.length}</p>
                  <p className="text-app-xs font-black uppercase" style={{ color: 'var(--col-success-60)' }}>Ready</p>
                </div>
                <div className="flex-1 text-center p-2 rounded-xl" style={{ background: 'var(--col-danger-08)', border: '1px solid var(--col-danger-25)' }}>
                  <p className="text-lg font-black" style={{ color: "var(--col-danger)" }}>{invalidRows.length}</p>
                  <p className="text-app-xs font-black uppercase" style={{ color: 'rgba(248,113,113,0.6)' }}>Errors</p>
                </div>
                <div className="flex-1 text-center p-2 rounded-xl" style={{ background: 'var(--rgba-white-05)', border: '1px solid var(--glass-border)' }}>
                  <p className="text-lg font-black" style={{ color: 'var(--text-primary)' }}>{rows.length}</p>
                  <p className="text-app-xs font-black uppercase" style={{ color: 'var(--text-muted)' }}>Total</p>
                </div>
              </div>
              {invalidRows.length > 0 && (
                <button onClick={() => setShowAll(v => !v)} className="w-full mt-3 py-2 rounded-xl text-app-sm font-black flex items-center justify-center gap-1.5 active:scale-95"
            ...
            {displayRows.map((row, i) => {
              ...
              return (
                <div key={real} className="rounded-2xl overflow-hidden" style={{ background: ok ? 'var(--col-success-06)' : 'var(--col-danger-07)', border: `1px solid ${ok ? 'var(--col-success-25)' : 'var(--col-danger-25)'}` }}>
                  <button className="w-full flex items-center gap-3 p-3" onClick={() => setExpanded(isExp ? null : real)}>
                    <div className="w-8 h-8 rounded-xl flex items-center justify-center flex-shrink-0" style={{ background: ok ? 'var(--col-success-15)' : 'var(--col-danger-15)' }}>
                      {ok ? <CheckCircle2 size={16} style={{ color: "var(--col-success)" }} /> : <AlertTriangle size={16} style={{ color: "var(--col-danger)" }} />}
                    </div>
                    <div className="flex-1 text-left">
                      <p className="text-xs font-black" style={{ color: ok ? "var(--col-success)" : "var(--col-danger)" }}>Row {real + 1}</p>
                  ...
                  {isExp && (
                    <div className="px-3 pb-3 space-y-1.5 border-t" style={{ borderColor: ok ? 'var(--col-success-15)' : 'var(--col-danger-25)' }}>
                      {row.errors.length > 0 && (
                        <div className="pt-2 space-y-1">
                          {row.errors.map((e, j) => <p key={j} className="text-app-sm font-bold" style={{ color: "var(--col-danger)" }}>• {e}</p>)}
                        </div>
                      )}
                      <div className="pt-1 grid grid-cols-2 gap-1">
                        {Object.entries(row.mapped).filter(([,v]) => v).map(([k, v]) => (
                          <div key={k} className="text-app-xs">
            ...
            {validRows.length > 0 && (
              <button onClick={runImport} disabled={importing} className="w-full py-4 rounded-2xl font-black text-sm flex items-center justify-center gap-2 active:scale-[0.97]"
                style={{ background: 'linear-gradient(135deg,#059669,#10b981)', color: 'white', boxShadow: '0 8px 24px var(--col-emerald-40)', opacity: importing ? 0.7 : 1 }}>
        ...
        {step === 'done' && result && (
          <div className="space-y-4">
            <div className="text-center py-8">
              <div className="w-20 h-20 rounded-full flex items-center justify-center mx-auto mb-4" style={{ background: 'var(--col-success-15)', border: '2px solid var(--col-success-35)' }}>
                <CheckCircle2 size={36} style={{ color: "var(--col-success)" }} />
              </div>
            ...
            {result.rowErrors.length > 0 && (
              <GCard>
                <p className="text-app-sm font-black uppercase tracking-widest mb-2" style={{ color: "var(--col-danger)" }}>Row Errors</p>
                {result.rowErrors.map((e, i) => <p key={i} className="text-app-sm mb-1" style={{ color: 'rgba(248,113,113,0.7)' }}>{e}</p>)}
              </GCard>
            )}
            ...
            <button onClick={onBack} className="w-full py-3 rounded-2xl font-black text-sm active:scale-95"
              style={{ background: 'var(--rgba-white-07)', color: 'var(--text-muted)' }}>← Back to Dashboard</button>
          </div>
        )}
      </div>
    </div>
