...
import { generatePartyCode, seedPartyCounter } from '../../utils/idGenerator';
...
const BulkImportView: React.FC<Props> = ({ user, settings, onBack }) => {
  ...
  const runImport = async () => {
    ...
    try {
      // Seed the party-code counter so new parties receive sequential P-XXXX tags
      if (type === 'parties') {
        try {
      ...
      for (let i = 0; i < validRows.length; i++) {
        try {
          ...
          if (type === 'parties' && !row.party_code) row.party_code = generatePartyCode();
