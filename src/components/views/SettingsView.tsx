...
const SECTIONS: { id: SettingsSection; label: string; sub: string; icon: React.ElementType; color: string; bg: string }[] = [
  { id: 'profile',      label: 'Firm Profile',      sub: 'Business name, address, GSTIN',   icon: UserIcon,       color: "var(--col-info)", bg: 'var(--col-info-12)' },
  { id: 'general',      label: 'General',            sub: 'Theme, currency, preferences',    icon: SettingsIcon,   color: "var(--col-violet)", bg: 'var(--col-violet-12)' },
  { id: 'invoice',      label: 'Invoice Template',   sub: 'Logo, footer, tax settings',      icon: FileTextIcon,   color: "var(--col-success)", bg: 'var(--col-emerald-12)' },
  { id: 'invitations',  label: 'Invitations & Firms', sub: 'Accept invites, switch firms',  icon: Building2,      color: "var(--col-pink)", bg: 'rgba(244,114,182,0.12)' },
  { id: 'users',        label: 'Users & Access',     sub: 'Staff accounts, permissions',     icon: Users,          color: "var(--col-pink)", bg: 'rgba(244,114,182,0.12)' },
  { id: 'lists',        label: 'Custom Lists',       sub: 'Categories, units, tags',         icon: List,           color: "var(--col-warning)", bg: 'var(--col-warning-12)' },
  { id: 'security',     label: 'Security',           sub: 'PIN lock, auto-lock timer',       icon: Lock,           color: "var(--col-danger)", bg: 'var(--col-danger-12)' },
  { id: 'data',         label: 'Data & Backup',      sub: 'Export, import, reset',           icon: Database,       color: "var(--col-slate)", bg: 'var(--text-muted)' },
  { id: 'subscription', label: 'Subscription',       sub: 'Free plan · Upgrade to Pro',      icon: Crown,          color: "var(--col-warning)", bg: 'var(--col-warning-12)' },
];
      ...
      <div className="h-full flex flex-col" style={{ background: 'var(--app-bg)' }}>
        ...
        <div className="shrink-0 px-4 pt-5 pb-4 flex items-center gap-3 border-b"
          ...
          <div className="flex-1 min-w-0">
            <h1 className="font-black text-lg text-[var(--text-primary)] tracking-tight">Settings</h1>
            <p className="text-app-sm text-[var(--text-muted)] truncate">{user.email}</p>
          </div>
        </div>
        ...
        <div ref={menuScrollRef} className="flex-1 overflow-y-auto px-4 py-4 space-y-2.5" style={{ WebkitOverflowScrolling: 'touch', paddingBottom: 'calc(96px + env(safe-area-inset-bottom, 0px))' }}>
          <p className="text-app-xs font-black uppercase tracking-[0.2em] text-[var(--text-muted)] px-1 mb-3">Configure your app</p>
          {SECTIONS.filter(s => !(s.id === 'subscription' && (globalConfig === null || globalConfig.appMode === 'free'))).map(({ id, label, sub, icon: Icon, color, bg }) => {
            ...
            return (
              <button key={id} onClick={() => handleSectionClick(id)}
                ...
                <div className="flex-1 min-w-0">
                  <p className="font-black text-sm text-[var(--text-primary)]">{label}</p>
                  <p className="text-app-sm text-[var(--text-muted)] truncate">{displaySub}</p>
                </div>
          ...
          <button onClick={() => onNavigate('staff')}
            ...
            <div className="w-11 h-11 rounded-[16px] flex items-center justify-center flex-shrink-0" style={{ background: 'rgba(124,58,237,0.12)' }}>
              <Users size={19} style={{ color: "var(--col-violet)" }} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-black text-sm" style={{ color: "var(--col-violet)" }}>Staff Management</p>
              <p className="text-app-sm text-[var(--text-muted)] truncate">Salary, advances, collections</p>
            </div>
            <ChevronRight size={16} style={{ color: 'var(--text-muted)', flexShrink: 0 }} />
          </button>
          ...
          <button onClick={() => onNavigate('daily-snapshots')}
            className="w-full flex items-center gap-4 p-4 rounded-[20px] active:scale-[0.98] transition-all text-left"
            style={{ background: 'var(--col-success-05)', border: '1px solid var(--col-success-15)' }}>
            <div className="w-11 h-11 rounded-[16px] flex items-center justify-center flex-shrink-0" style={{ background: 'var(--col-success-15)' }}>
              <HardDrive size={19} style={{ color: "var(--col-success)" }} />
            </div>
            <div className="flex-1 min-w-0">
              <p className="font-black text-sm" style={{ color: "var(--col-success)" }}>Daily Data Archive</p>
              <p className="text-app-sm text-[var(--text-muted)] truncate">Last 7 days · CSV · Survives reinstall</p>
            </div>
            <ChevronRight size={16} style={{ color: 'var(--col-success-35)', flexShrink: 0 }} />
          </button>
          ...
          {isSuperAdmin && (
            <button onClick={() => handleSectionClick('developer')}
              className="w-full flex items-center gap-4 p-4 rounded-[20px] active:scale-[0.98] transition-all text-left"
              style={{ background: 'var(--col-accent-05)', border: '1px solid var(--col-accent-18)' }}>
              <div className="w-11 h-11 rounded-[16px] flex items-center justify-center flex-shrink-0" style={{ background: 'var(--col-accent-12)' }}>
                <Terminal size={19} style={{ color: "var(--col-indigo)" }} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-black text-sm" style={{ color: "var(--col-indigo)" }}>Developer Console</p>
                <p className="text-app-sm text-[var(--text-muted)] truncate">Crash logs · Download · Mark resolved</p>
              </div>
              <ChevronRight size={16} style={{ color: 'var(--col-accent-35)', flexShrink: 0 }} />
            </button>
          )}
          ...
          <div className="pt-4 space-y-2">
            <button onClick={handleLogout}
              className="w-full flex items-center gap-4 p-4 rounded-[20px] active:scale-[0.98] transition-all text-left"
              style={{ background: 'var(--col-danger-06)', border: '1px solid var(--col-danger-12)' }}>
              <div className="w-11 h-11 rounded-[16px] flex items-center justify-center flex-shrink-0" style={{ background: 'var(--col-danger-15)' }}>
                <LogOut size={19} style={{ color: "var(--col-danger)" }} />
              </div>
              <div className="flex-1 min-w-0">
                <p className="font-black text-sm" style={{ color: "var(--col-danger)" }}>Logout</p>
                <p className="text-app-sm text-[var(--text-muted)]">Sign out of your account</p>
              </div>
            </button>
            <p className="text-center text-app-xs text-[var(--text-muted)] font-bold uppercase tracking-widest pt-2">
  ...
  return (
    <div className="h-full flex flex-col" style={{ background: 'var(--app-bg)' }}>
      ...
      <div className="shrink-0 px-4 pt-5 pb-4 flex items-center gap-3 border-b"
        ...
        <div className="flex-1 min-w-0">
          <h1 className="font-black text-base text-[var(--text-primary)] tracking-tight">
            {currentSection?.label ?? 'Settings'}
          </h1>
          <p className="text-app-sm text-[var(--text-muted)] truncate">
            {currentSection?.sub}
          </p>
        </div>
        {needsSave && (
