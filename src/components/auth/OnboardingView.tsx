...
const OnboardingView: React.FC<OnboardingViewProps> = ({ onComplete }) => {
  ...
  const handleGstFetch = async () => {
    ...
    try {
      ...
      const name    = res.legalName || res.tradeName || '';
      const address = res.address   || '';
      const state   = res.state     || '';
      const filled: string[] = [];

      // Always override all fields returned by the API
      setFirm(prev => {
        const next = { ...prev };
        if (name)    { next.firm_name = name;    filled.push('Business Name'); }
        if (address) { next.address   = address; filled.push('Address'); }
        if (state)   { next.state     = state;   filled.push('State'); }
        return next;
      });
      setGstFetchResult({ ...res, legalName: name, filledFields: filled });
    } catch (e: any) {
