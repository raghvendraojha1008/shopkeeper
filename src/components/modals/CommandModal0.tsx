  ...
  return (
    <>
        <div className="fixed inset-0 bg-black/80 z-50 flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="command-modal-root w-full max-w-2xl rounded-3xl overflow-hidden shadow-2xl border border-white/10 flex flex-col max-h-[90vh]" style={{background:"var(--app-bg)"}}>
                
                {/* Header */}
                <div className="p-4 border-b border-white/08 flex justify-between items-center" style={{background:"var(--rgba-white-03)"}}>
                    <div className="flex items-center gap-2">
                        <div className="p-2 rounded-lg bg-[var(--col-violet-18)] text-violet-400">
                            <Sparkles size={18} />
                        </div>
                        <div>
                    ...
                    <button onClick={onClose} className="p-2 hover:bg-slate-200 hover:bg-[var(--rgba-white-08)] rounded-full text-[var(--text-primary)]"><X size={20}/></button>
                </div>

                {error && (
                    <div className="bg-[var(--col-danger-12)] p-3 flex gap-2 items-center text-col-danger text-xs font-bold px-6">
                        <AlertTriangle size={14}/> {error}
                    </div>
                )}

                <div className="p-4 flex-1 overflow-y-auto min-h-[300px]" style={{background:"var(--app-bg)"}}>
                    {scannedData ? (
                        <div className="space-y-3">
                            <div className="bg-[var(--col-emerald-10)] p-3 rounded-xl flex items-center gap-2 text-col-success text-sm font-bold border border-[var(--col-emerald-25)]">
                                <CheckCircle2 size={18} />
                                {scannedData.length} Entries Extracted. Review below.
                            </div>

                            {scannedData.map((item, idx) => (
                                <div key={idx} className="bg-[var(--rgba-white-06)] p-4 rounded-xl border border-white/12 flex justify-between items-center group hover:border-violet-400/30 transition-all">
                                    <div className="flex items-center gap-3">
                                        <div className={`p-2.5 rounded-xl shrink-0 ${item.type === 'sell' || item.type === 'received' ? 'bg-[var(--col-emerald-15)] text-col-success' : 'bg-[var(--col-danger-12)] text-col-danger'}`}>
                                            {item.type === 'sell' ? <Package size={18}/> : <ArrowRight size={18}/>}
                                        </div>
                                        <div>
                                            <div className="flex items-center gap-2 mb-1">
                                                <span className="text-app-sm font-black uppercase border border-white/15 px-2 py-0.5 rounded tracking-wider">
                                    ...
                                    <div className="flex items-center gap-2">
                                        <button 
                                            ...
                                            className="p-2 bg-[var(--col-info-12)] text-col-info rounded-lg hover:bg-[var(--col-info-25)] transition-colors"
                                            title="Edit & Verify"
                                        >
                                            <Edit2 size={16} />
                                        </button>
                                        <button 
                                            ...
                                            className="p-2 bg-[var(--col-danger-12)] text-col-danger rounded-lg hover:bg-[var(--col-danger-25)] transition-colors"
                                        >
                                            <Trash2 size={16} />
                                        </button>
                                    </div>
                        ...
                        <div className="space-y-4">
                            <textarea 
                                className="w-full h-48 p-4 border border-white/12 bg-[var(--rgba-white-05)] rounded-xl resize-none outline-none focus:ring-2 focus:ring-blue-500 font-medium text-sm transition-all"
                                placeholder="e.g. 'Sold 50 cement bags to Rahul for 15000' or 'Paid 500 for Tea'"
                                value={input}
                                onChange={e => setInput(e.target.value)}
                            />
                            
                            <label className="flex items-center justify-center gap-2 w-full p-6 border-2 border-dashed border-white/10 rounded-xl cursor-pointer hover:bg-[var(--rgba-white-08)] transition-colors">
                                <input type="file" className="hidden" accept="image/*,audio/*" onChange={e => setFile(e.target.files?.[0] || null)} />
                                <ImageIcon size={24} className="" style={{color: "var(--text-muted)"}} />
                                <span className="text-sm font-bold text-[var(--text-muted)]">
                                    {file ? file.name : "Upload Invoice Image or Audio Note"}
                                </span>
                            </label>
                            
                            <p className="text-center text-app-sm text-[var(--text-muted)]">
                    ...
                    {scannedData ? (
                        <>
                            <button 
                                ...
                                className="px-6 py-3 rounded-xl font-bold text-[var(--text-muted)] hover:bg-slate-200 transition-colors"
                            >
                                Back
                            </button>
                            <button 
