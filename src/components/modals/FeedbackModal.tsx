...
const TABS: Array<{ id: FeedbackType; label: string; icon: React.ElementType; color: string }> = [
  { id: 'bug',     label: 'Report Bug',     icon: Bug,      color: "var(--col-red)" },
  { id: 'feature', label: 'Suggest Idea',   icon: Sparkles, color: "var(--col-violet)" },
  { id: 'rating',  label: 'Rate App',       icon: Star,     color: "var(--col-warning)" },
];
...
export const FeedbackModal: React.FC<Props> = ({ open, onClose, userId, screen }) => {
  ...
  return (
    <div
      ...
      style={{ background: 'var(--rgba-black-60)', backdropFilter: 'blur(8px)' }}
      onClick={handleClose}
    >
      <div
      ...
      >
        {/* Header */}
        <div className="flex items-center justify-between px-5 py-4 border-b" style={{ borderColor: 'var(--rgba-white-06)' }}>
          <div>
            <h2 className="text-lg font-black text-white">Send Feedback</h2>
            <p className="text-app-md mt-0.5" style={{ color: 'var(--text-muted)' }}>
            ...
            {TABS.map(tab => {
              ...
              return (
                <button
                  ...
                  style={{
                    background: active ? `${tab.color}1f` : 'var(--rgba-white-04)',
                    border    : `1px solid ${active ? tab.color + '55' : 'var(--rgba-white-06)'}`,
                  }}
                >
                  <Icon size={16} style={{ color: active ? tab.color : 'var(--text-muted)' }} />
                  <span className="text-app-sm font-bold" style={{ color: active ? tab.color : 'var(--text-muted)' }}>
                    {tab.label}
                  </span>
                </button>
              );
        ...
        <div className="px-5 py-5 flex-1 overflow-y-auto">
          ...
          {type === 'rating' && (
            <div className="mb-4">
              <label className="text-app-sm font-black uppercase tracking-wider block mb-2"
                ...
                {[1, 2, 3, 4, 5].map(n => (
                  ...
                  >
                    <Star
                      ...
                      strokeWidth={1.5}
                      style={{
                        color: n <= rating ? "var(--col-warning)" : 'var(--text-muted)',
                        fill : n <= rating ? "var(--col-warning)" : 'transparent',
                      }}
                    />
                  </button>
                ))}
          ...
          />
          <p className="text-app-xs mt-1.5 text-right" style={{ color: 'var(--text-muted)' }}>
            {message.length}/2000
          </p>
        </div>

        {/* Footer */}
        <div className="px-5 py-4 border-t" style={{ borderColor: 'var(--rgba-white-06)' }}>
          <button
            ...
            style={{ background: 'linear-gradient(135deg, #6366f1, #8b5cf6)', boxShadow: '0 8px 20px var(--col-accent-35)' }}
          >
            {submitting ? (
