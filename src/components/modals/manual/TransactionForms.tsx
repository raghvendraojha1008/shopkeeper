...
import {
  ...
  CheckCircle2, Clock, AlertCircle, Package, Link, Repeat
} from 'lucide-react';
    ...
    <>
      ...
      <div className="pt-1">
        <button
          ...
          className="w-full flex items-center justify-between p-3 rounded-xl active:scale-[0.98] transition-all"
          style={{ background: formData.isRecurring ? 'rgba(99,102,241,0.12)' : 'rgba(255,255,255,0.04)', border: `1px solid ${formData.isRecurring ? 'rgba(99,102,241,0.3)' : 'rgba(255,255,255,0.08)'}` }}>
          <div className="flex items-center gap-2.5">
            <div className="p-1.5 rounded-lg" style={{ background: formData.isRecurring ? 'rgba(99,102,241,0.2)' : 'rgba(255,255,255,0.06)' }}>
              <Repeat size={13} style={{ color: formData.isRecurring ? '#a78bfa' : 'rgba(148,163,184,0.5)' }} />
            </div>
            <div className="text-left">
              <p className="text-xs font-black" style={{ color: formData.isRecurring ? 'rgba(226,232,240,0.9)' : 'rgba(148,163,184,0.6)' }}>Recurring</p>
              <p className="text-[9px]" style={{ color: 'rgba(148,163,184,0.4)' }}>Auto-create on schedule</p>
            </div>
          </div>
          <div className={`w-10 h-5 rounded-full transition-all relative flex-shrink-0 ${formData.isRecurring ? 'bg-indigo-500' : 'bg-white/10'}`}>
            <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full shadow transition-all duration-200 ${formData.isRecurring ? 'left-[22px]' : 'left-0.5'}`} />
          </div>
        </button>
          ...
          <div className="mt-2 grid grid-cols-3 gap-2 animate-in slide-in-from-top-2 duration-200">
            {(['daily', 'weekly', 'monthly'] as const).map(interval => (
              <button
                ...
                className="py-2 rounded-xl text-[10px] font-black capitalize transition-all active:scale-95"
                style={formData.recurringInterval === interval
                  ? { background: 'rgba(99,102,241,0.2)', color: '#a78bfa', border: '1px solid rgba(99,102,241,0.4)' }
