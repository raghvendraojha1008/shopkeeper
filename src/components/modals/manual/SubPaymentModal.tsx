...
import { X, Save, IndianRupee, FileText, Wallet, AlertCircle, UserCheck } from 'lucide-react';
import { InputField, AutoComplete } from './FormUI';
import { useUI } from '../../../context/UIContext'; // Import UI Context

export const SubPaymentModal = ({ isOpen, onClose, onAdd, defaultData, paymentModes, purposes, maxAmount, isSale, receivedByNames, paidByNames }: any) => {
    ...
    return (
        <div className="fixed inset-0 bg-black/60 z-[70] flex justify-center items-center p-4 backdrop-blur-sm">
            <div className="w-full max-w-md rounded-2xl shadow-2xl border border-white/10 flex flex-col animate-in zoom-in-95 duration-200" style={{background:"var(--app-bg)"}}>
                <div className="p-4 border-b border-white/08 flex justify-between items-center  rounded-t-2xl">
                    <h3 className="font-bold text-[var(--text-secondary)] text-[var(--text-primary)]">Add Payment</h3>
                    <button onClick={onClose} className="p-1 rounded-full hover:bg-slate-200 hover:bg-[var(--rgba-white-08)]"><X size={18}/></button>
                </div>
                
                <div className="p-5 space-y-4">
                    <div className="bg-[var(--col-info-08)] p-3 rounded-xl border border-[var(--col-info-25)] space-y-2 text-xs">
                        <div className="flex justify-between"><span className="text-slate-500 font-bold">Party:</span><span className="font-bold text-[var(--text-primary)]">{data.party_name}</span></div>
                        <div className="flex justify-between items-center"><span className="text-slate-500 font-bold">Ref #:</span><span className="font-black bg-white bg-[var(--rgba-white-06)] px-2 py-0.5 rounded border border-white/10 text-[var(--text-primary)]">{data.bill_no}</span></div>
                        {maxAmount > 0 && (
                    ...
                    <div className="grid grid-cols-2 gap-3">
                         ...
                         <div className="mb-3">
                            <label className="block text-xs font-bold text-[var(--text-muted)] mb-1">Mode</label>
                            <select className="w-full border border-white/12 bg-[var(--rgba-white-05)] p-2.5 rounded-lg text-sm font-bold text-[var(--text-secondary)] h-[42px]" value={data.payment_mode} onChange={e => setData({...data, payment_mode: e.target.value})}>
                    ...
                    <AutoComplete
                        label={isSale ? 'Received By' : 'Paid By'}
                        value={data.received_by || ''}
                        onChange={(v: string) => setData({...data, received_by: v})}
                        options={isSale
                            ? (receivedByNames || ['Owner', 'Manager', 'Staff'])
                            : (paidByNames    || ['Owner', 'Manager'])
                        }
                        icon={UserCheck}
                        placeholder={isSale ? 'Who collected payment?' : 'Who made payment?'}
                    />
                </div>
