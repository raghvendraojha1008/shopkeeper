...
export const RecurringService = {
  ...
  async checkAndProcess(uid: string): Promise<void> {
    ...
    try {
      ...
      // Seed ID counters so auto-generated IDs don't collide with existing ones.
      try {
        const snap = await getDocs(txnCol);
        seedCountersFromFirestore(
          snap.docs.filter(d => d.data().type === 'received').map(d => d.data().transaction_id),
          'receipts'
        );
        seedCountersFromFirestore(
          snap.docs.filter(d => d.data().type === 'paid').map(d => d.data().transaction_id),
          'payments'
        );
      } catch { /* non-fatal */ }
      ...
      for (const tmpl of due) {
        const transaction_id = generatePrefixedID(tmpl.type === 'paid' ? 'payments' : 'receipts');
        await addDoc(txnCol, {
          date: today,
          type: tmpl.type,
