...
/**
 * Generate a Firestore-compatible random document ID client-side.
 * Matches Firestore's own auto-ID format: 20 chars from [A-Za-z0-9].
 * Used to pre-generate IDs before writes so that both a Firestore SDK
 * in-memory pending write and a SyncQueueService replay target the same
 * document, making the combined write idempotent (no duplicates).
 */
 ...
 * WRITE TIMEOUT GUARD
 * ─────────────────────────────────────────────────────────────────────────────
 * On Android WebView under memory pressure, Firestore's persistentLocalCache
 * writes to IndexedDB can hang indefinitely — the Promise never resolves AND
 * never rejects. This leaves any UI that does `setLoading(true)` → `await write`
 * → `setLoading(false)` frozen on "Saving…" forever.
 *
 * This wrapper races every write against a hard 15-second timeout. If the
 * timeout fires first we reject with a structured WRITE_TIMEOUT error so the
 * caller can fall back to the offline sync-queue instead of hanging the UI.
 * 15 s is generous — a healthy IndexedDB write resolves in < 500 ms.
 */
...
export const ApiService = {
  ...
  /**
   * Write a document to a specific pre-generated ID using setDoc.
   * Idempotent: if the Firestore SDK's in-memory pending write and a
   * SyncQueueService replay both call this with the same id and data,
   * the second write simply overwrites with identical data — no duplicate.
   */
  setDocById: async (
    uid: string,
    col: string,
    ...
    await withWriteTimeout(
      setDoc(doc(db, `users/${uid}/${col}`, id), sanitizeForFirestore(data)),
    );
    const itemName = data.name || data.party_name || data.item_name || data.invoice_no || 'Item';
    auditAsync(uid, 'create', col, id, `Created ${col.replace('_', ' ')}: ${itemName}`, {
  ...
  batchSave: async (
    ...
    for (const op of operations) {
      if (op.type === 'add') {
        // If the caller pre-generated an ID (to make the write idempotent),
        // use setDoc to that specific document.  Otherwise auto-generate.
        const ref = op.id
          ? doc(db, `users/${uid}/${op.col}`, op.id)
          : doc(collection(db, `users/${uid}/${op.col}`));
