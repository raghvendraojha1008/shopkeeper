...
async function saveDetailedCsvFiles(
  ...
  const partyHeaders = ['id', 'name', 'role', 'contact', 'gstin', 'address', 'city', 'state', 'site', 'credit_limit'];
  const partyRows = (data['parties'] || []).map((p: any) => [
    p.id, p.name, p.role, p.contact, p.gstin, p.address, p.city, p.state, p.site, p.credit_limit ?? '',
  ]);
