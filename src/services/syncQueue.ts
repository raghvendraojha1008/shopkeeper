      ...
      for (let i = 0; i < sorted.length; i++) {
        ...
        try {
          if (item.operation === 'create') {
            if (item.docId) {
              // Pre-generated client-side ID — use setDoc so the write is
              // idempotent.  If the Firestore SDK's in-memory pending write
              // already completed when connectivity returned, this setDoc
              // overwrites with identical data rather than creating a second
              // duplicate document via addDoc.
              await ApiService.setDocById(userId, item.collection, item.docId, item.data);
            } else {
