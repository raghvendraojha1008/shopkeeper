...
type IDCategory = 'sales' | 'purchases' | 'customers' | 'suppliers' | 'inventory' | 'receipts' | 'payments' | 'waste' | 'expenses' | 'services';
 ...
 *   Suppliers → S-0001, S-0002, …
 * Sequential, auto-assigned and immutable after first save.
 * Legacy P-XXXX codes (from previous versions) are accepted on seed.
 * ───────────────────────────────────────────────────────────────────────────── */

let _customerCodeCounter = 0;
let _supplierCodeCounter = 0;
const _existingCustomerCodes = new Set<number>();
const _existingSupplierCodes = new Set<number>();

/** Parse the numeric part from a party code string (handles C-, S-, P- prefixes). */
function parsePartyCodeNum(code: string): number {
  const n = parseInt(String(code).replace(/^[CSPcsp]-0*/,''), 10);
  return isNaN(n) ? 0 : n;
}

/**
 * Seed from existing party_code values so counters never go backwards.
 * Detects C- (customer), S- (supplier), and legacy P- codes automatically.
 */
export const seedPartyCounter = (existingCodes: (string | undefined | null)[]): void => {
  let maxC = _customerCodeCounter;
  let maxS = _supplierCodeCounter;
  existingCodes.forEach(code => {
    if (!code) return;
    const str = String(code).toUpperCase();
    const n   = parsePartyCodeNum(str);
    if (!n) return;
    if (str.startsWith('S-')) {
      _existingSupplierCodes.add(n);
      if (n > maxS) maxS = n;
    } else {
      // C- prefix AND legacy P- prefix both go to the customer counter
      // (P- was the old single-pool prefix; treat as customers so no gaps collide)
      _existingCustomerCodes.add(n);
      if (n > maxC) maxC = n;
    }
  });
  if (maxC > _customerCodeCounter) _customerCodeCounter = maxC;
  if (maxS > _supplierCodeCounter) _supplierCodeCounter = maxS;
};

/** Peek the next party code without advancing the counter (use for pre-filling the form). */
export const peekNextPartyCode = (role: 'customer' | 'supplier' = 'customer'): string => {
  if (role === 'supplier') {
    let next = _supplierCodeCounter + 1;
    let scanned = 0;
    while (_existingSupplierCodes.has(next) && scanned < MAX_GAP_SCAN) { next++; scanned++; }
    return `S-${String(next).padStart(4, '0')}`;
  }
  let next = _customerCodeCounter + 1;
  let scanned = 0;
  while (_existingCustomerCodes.has(next) && scanned < MAX_GAP_SCAN) { next++; scanned++; }
  return `C-${String(next).padStart(4, '0')}`;
};

/** Generate and advance the counter (use on actual save). */
export const generatePartyCode = (role: 'customer' | 'supplier' = 'customer'): string => {
  if (role === 'supplier') {
    let next = _supplierCodeCounter + 1;
    let scanned = 0;
    while (_existingSupplierCodes.has(next) && scanned < MAX_GAP_SCAN) { next++; scanned++; }
    _supplierCodeCounter = next;
    _existingSupplierCodes.add(next);
    return `S-${String(next).padStart(4, '0')}`;
  }
  let next = _customerCodeCounter + 1;
  let scanned = 0;
  while (_existingCustomerCodes.has(next) && scanned < MAX_GAP_SCAN) { next++; scanned++; }
  _customerCodeCounter = next;
  _existingCustomerCodes.add(next);
  return `C-${String(next).padStart(4, '0')}`;
};

/** Confirm a party code after save so the counter stays in sync. */
export const confirmPartyCode = (code: string): void => {
  ...
  if (!n) return;
  if (str.startsWith('S-')) {
    if (n > _supplierCodeCounter) _supplierCodeCounter = n;
    _existingSupplierCodes.add(n);
  } else {
    if (n > _customerCodeCounter) _customerCodeCounter = n;
    _existingCustomerCodes.add(n);
  }
};

/* ─────────────────────────────────────────────────────────────────────────────
 * Staff Code utilities  (ST-001, ST-002, …)
 * ───────────────────────────────────────────────────────────────────────────── */

let _staffCodeCounter = 0;
const _existingStaffCodes = new Set<number>();

export const seedStaffCounter = (existingCodes: (string | undefined | null)[]): void => {
  let max = _staffCodeCounter;
  existingCodes.forEach(code => {
    if (!code) return;
    const n = parseInt(String(code).replace(/^ST-0*/i, ''), 10);
    if (!isNaN(n) && n > 0) { _existingStaffCodes.add(n); if (n > max) max = n; }
  });
  if (max > _staffCodeCounter) _staffCodeCounter = max;
};

export const peekNextStaffCode = (): string => {
  let next = _staffCodeCounter + 1;
  let scanned = 0;
  while (_existingStaffCodes.has(next) && scanned < MAX_GAP_SCAN) { next++; scanned++; }
  return `ST-${String(next).padStart(3, '0')}`;
};

export const generateStaffCode = (): string => {
  let next = _staffCodeCounter + 1;
  let scanned = 0;
  while (_existingStaffCodes.has(next) && scanned < MAX_GAP_SCAN) { next++; scanned++; }
  _staffCodeCounter = next;
  _existingStaffCodes.add(next);
  return `ST-${String(next).padStart(3, '0')}`;
};

export const confirmStaffCode = (code: string): void => {
  const n = parseInt(String(code).replace(/^ST-0*/i, ''), 10);
  if (isNaN(n) || !n) return;
  if (n > _staffCodeCounter) _staffCodeCounter = n;
  _existingStaffCodes.add(n);
};
