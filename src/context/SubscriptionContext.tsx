...
import { doc, collection, setDoc, Timestamp, onSnapshot, getDocs, getDoc } from 'firebase/firestore';
...
export const SubscriptionProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  // Fetched ONCE per session with a 1-hour localStorage cache.
  // Real-time listeners are not needed here — admin changes to plans/config
  // are picked up on the next app open (or cache expiry).
  // This eliminates two persistent Firestore read streams per active session.
  useEffect(() => {
    let mounted = true;

    const PLAN_KEY   = 'sk_plans_v1';
    const CONFIG_KEY = 'sk_config_v1';
    ...
    // ── Plans ──────────────────────────────────────────────────────────────
    const cachedPlans = readCache<Plan[]>(PLAN_KEY);
    if (cachedPlans) {
      if (mounted) setPlans(cachedPlans);
    } else {
      getDocs(collection(db, 'plans'))
        .then(snap => {
          if (!mounted) return;
          const plans = snap.empty
            ? Object.entries(DEFAULT_PLANS).map(([id, p]) => ({ id, ...p } as Plan))
            : snap.docs.map(d => ({ id: d.id, ...d.data() } as Plan));
          setPlans(plans);
          writeCache(PLAN_KEY, plans);
        })
        .catch(() => {
    ...
    // ── Global config ──────────────────────────────────────────────────────
    // Safety timeout: if Firestore never responds, unblock the loading gate.
    const configTimeoutId = setTimeout(() => {
      if (mounted) setGlobalConfigLoaded(true);
    }, 4000);

    const cachedConfig = readCache<GlobalConfig>(CONFIG_KEY);
    if (cachedConfig) {
      if (mounted) {
        setGlobalConfig(cachedConfig);
        setGlobalConfigLoaded(true);
        clearTimeout(configTimeoutId);
        gracePeriodMsRef.current = cachedConfig.gracePeriodDays * 24 * 60 * 60 * 1000;
        globalConfigRef.current  = cachedConfig;
      }
    } else {
      getDoc(doc(db, 'config', 'global'))
        .then(snap => {
          ...
          globalConfigRef.current  = data;
          writeCache(CONFIG_KEY, data);
        })
        .catch(() => {
          if (mounted) {
            setGlobalConfigLoaded(true);
